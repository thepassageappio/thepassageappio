...
const takeaways = [
  ...
  ['Family, care, participant, and vendor UX', 'Production/source pass active', 'B2C, urgent, planning, hospice/care-prep, participant, vendor, and care-provider flows now favor family record, request, next step, owner, waiting point, proof, and scoped access. Latest queued source makes the family coordinator plan feel structurally and visually different: the first action and tier rows are operating-step cards, opened steps become a family operating sheet with owner, waiting, prepared output, proof, visibility, review-before-send messages, and one primary action lane, and estate detail steps now open as an Estate operating sheet instead of a dense task update panel. Participant and vendor surfaces now say own the request, show what is waiting, save completion proof, and approve quote before work starts instead of accept/mark-completed/task language. Vendor lifecycle labels now say Quote Approved, approval/payment next step, and approve-and-pay; scoped vendor request pages no longer say family/funeral home accepts the quote, and both visible and legacy vendor response paths now prevent schedule/complete actions before approval/payment. Remaining source work: participant/vendor detail surfaces and Chrome visual QA need the same non-cosmetic operating-lane standard before the persona overhaul is complete.'],
