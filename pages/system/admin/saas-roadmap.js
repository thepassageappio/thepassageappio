...
const takeaways = [
  ...
  ['Green-path onboarding lookup', 'Production clarity fixed / config still required', 'Pre-need and case address entry now explains that typed addresses are valid even when Google suggestions do not appear, and the shared address input labels the fallback as Use this typed address. Google suggestions still require the Vercel production server key and should be rechecked without client-side extension blocking before marking lookup UAT complete.'],
