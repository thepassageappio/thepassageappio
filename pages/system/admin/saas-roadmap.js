...
const takeaways = [
  ...
  ['Live public UAT sweep', 'P0 release routing', 'Latest main source is queued but both GitHub production contexts are currently Vercel rate-limited. www.thepassageapp.io previously served older homepage copy including sample funeral-home console and Spine labels. Treat custom domain, alias, Cloudflare, Vercel duplicate-context cleanup, and cache routing as P0 before LinkedIn promotion, paid pilots, or final persona QA signoff. Public readiness compares X-Passage-Commit against the current Vercel commit and source-checks client-rendered persona flows so stale pages or role-copy regressions become explicit blockers.'],
  ...
  ['Funeral-home product UX', 'Source simplified and guarded / deploy verification pending', 'Director/staff dashboard now uses Case at a glance, Recommended next action, work-card contracts, one primary action, one waiting action, collapsed More actions, what Passage prepared, what staff does next, waiting point, proof saves, needs-help wording, family-visible proof, and customer-safe setup, invite, queue, vendor preference, and billing language with visible pilot/trial wording, quote-accepted labels, and raw plan ids removed from the customer dashboard. Setup now uses funeral-home/customer language, a logged-out recommended next action, a required-info gate, direct demo-dashboard access, and first next-step/work-card wording instead of partner/task wording. Inbound family requests now show clean contact separators and operator-safe accept/decline/convert controls. Authenticated UAT must verify My Day and work cards once browser/domain access is usable.'],
  ['Family, care, participant, and vendor UX', 'Source simplified and guarded / deploy blocked by rate limit', 'B2C, urgent, planning, hospice/care-prep, participant, vendor, and care-provider flows now favor family record, request, next step, owner, waiting point, proof, and scoped access. Family homepage/dashboard copy now says who owns each next step, prepared output, proof, active plan, and next-step lists without broken placeholder separators. Participant and vendor surfaces now say own the request, show what is waiting, save completion proof, and approve quote before work starts instead of accept/mark-completed/task language. Vendor lifecycle labels now say Quote Approved, approval/payment next step, and approve-and-pay; scoped vendor request pages no longer say family/funeral home accepts the quote. Vendor onboarding, care-provider inquiry, hospice/care-prep, contact, and funeral-home public copy now use provider/care-team/waiting-point language and avoid vague partner/stuck/internal wording. Live www must still be rechecked after routing/cache is fixed and Vercel deploys resume.'],
...
const sprints = [
  ...
  },
  {
    ...
    status: 'Source simplified and guarded / live domain stale',
    ...
    tasks: [
      ...
      'Browser-verify My Day, Case at a glance, Recommended next action, Request family info, Record proof / close, Mark waiting, inbound family requests, and work-card copy once Chrome/native bridge and domain routing are usable.',
    ],
  ...
  },
  {
    ...
    status: 'Active source UAT / Vercel rate-limited',
