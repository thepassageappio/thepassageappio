...
const takeaways = [
  ...
  ['Green-path onboarding lookup', 'Source fixed / batched deploy queued', 'Address entry now has a Passage-controlled fallback even when Google Maps is configured: server suggestions still run, Google script failures are handled, and users can choose Use typed address instead of getting stuck. Recheck urgent green-path, funeral-home setup, vendor onboarding, and any care-provider address capture once Vercel deploys.'],
