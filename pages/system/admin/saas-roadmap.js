...
const takeaways = [
  ...
  ['Family, care, participant, and vendor UX', 'Production/source pass active', 'B2C, urgent, planning, hospice/care-prep, participant, vendor, and care-provider flows now favor family record, request, next step, owner, waiting point, proof, and scoped access. Family homepage/dashboard/workspace copy now says who owns each next step, prepared output, proof, active plan, request details, open items, and next-step lists without broken placeholder separators or visible task-board language. Participant and vendor surfaces now say own the request, show what is waiting, save completion proof, and approve quote before work starts instead of accept/mark-completed/task language. Vendor lifecycle labels now say Quote Approved, approval/payment next step, and approve-and-pay; scoped vendor request pages no longer say family/funeral home accepts the quote, and both visible and legacy vendor response paths now prevent schedule/complete actions before approval/payment. Vendor onboarding, care-provider inquiry, hospice/care-prep, contact, participant landing, and funeral-home public copy now use provider/care-team/request/waiting-point language and avoid vague partner/stuck/internal wording. Participant scoped-access copy now says request/timing/proof instead of task, family workspace and urgent first-hour flows now say request/open item/proof instead of task-board language, shared playbook/workspace copy now avoids partner/task leakage in prepared outputs, and care-prep dates stay visible as waiting items instead of tasks. Vendor request flow now requires the proper sequence: send quote, family/funeral-home approval and payment, mark scheduled, then save completion proof; the request UI, initial vendor email, response API sequence gates, and invalid-link handling, required quote amounts, required scheduling detail, and required completion proof now avoid premature completion actions, payment dead ends, and raw database errors. Live www must still be rechecked after routing/cache is fixed and Vercel deploys resume.'],
