...
const takeaways = [
  ...
  ['Live public UAT sweep', 'P0 release routing', 'Latest main source is deploying cleanly through Vercel, but www.thepassageapp.io previously served older homepage copy including sample funeral-home console and Spine labels. Treat custom domain, alias, Cloudflare, and cache routing as P0 before LinkedIn promotion, paid pilots, or final persona QA signoff. Public readiness compares X-Passage-Commit against the current Vercel commit and source-checks client-rendered persona flows so stale pages or role-copy regressions become explicit blockers.'],
  ...
  ['Task-card and request-card clarity', 'Source hardened / deploy blocked by rate limit', 'Family and funeral-home cards now use a concrete operator contract: 1. do next, owner, waiting party, message to send or use, status/proof, and privacy boundary. Shared action confirmations, exports, owner assignment hints, bulk assignment notices, and expected-update copy now default to next-step/request/work-item language so internal task wording is less likely to leak across personas. Public/vendor/helper wording favors request, record, owner, waiting point, proof, and next step; participant confirmations and dialogs now say request instead of task, and vendor/care-provider copy no longer leaks task wording or noisy payout-fee language into scoped external requests. Latest deploy still needs custom-domain/browser verification after www routing and Chrome automation are corrected.'],
