...
import { buildContinuityPackets, buildContinuityContext, demoContinuityInput } from '../../lib/continuityPackets';
...
export default async function handler(req, res) {
  ...
  if (!id || req.query.demo === '1') {
    const input = demoContinuityInput();
    return res.status(200).json({ source: 'demo', context: buildContinuityContext(input), packets: buildContinuityPackets(input) });
  }

  const admin = adminClient();
  if (!admin) return res.status(500).json({ error: 'Supabase service role is not configured.' });
  const user = await getUser(req);
  if (!user?.id) return res.status(401).json({ error: 'Please sign in to generate packets from this case.' });
  ...
  if (!workflow || workflow.status === 'archived') return res.status(404).json({ error: 'Case not found.' });
  if (!(await canAccessWorkflow(admin, user, workflow))) return res.status(403).json({ error: 'You do not have access to this case.' });
