...
function enforceProcessorLimit(req) {
  ...
  return rateLimit({
    key: ['process-task-reminders', getRequestIp(req), String(req.headers.authorization || req.headers['x-passage-internal-secret'] || 'internal').slice(0, 24)].join(':'),
...
export default async function handler(req, res) {
  ...
  if (!limit.allowed) {
    ...
    return res.status(429).json({ error: 'Task reminder processing is cooling down. Wait before running it again.', retryAfterSeconds: limit.retryAfterSeconds });
  }
