      ...
      <section style={{ maxWidth: 1040, margin: '0 auto', padding: '28px 24px 54px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,.82fr) minmax(320px,1fr)', gap: 18, alignItems: 'start' }}>
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: 24, boxShadow: '0 12px 34px rgba(55,45,35,.055)' }}>
            ...
            <h1 style={{ fontSize: 'clamp(34px,4.6vw,56px)', lineHeight: .98, margin: '10px 0 12px', fontWeight: 400 }}>Local help, only when the task needs it.</h1>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {cards.map((card) => {
              ...
              const style = { display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 14, alignItems: 'center', background: card.tone === 'primary' ? C.sageFaint : C.card, border: `1px solid ${card.tone === 'primary' ? C.sage + '33' : C.border}`, borderRadius: 16, padding: '16px 17px', textDecoration: 'none', color: C.ink, boxShadow: '0 8px 24px rgba(55,45,35,.04)' };
              if (card.onClick) {
