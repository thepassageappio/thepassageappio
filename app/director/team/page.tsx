  ...
  return (
    <AppFrame active="team" identity={viewer.displayName} mode="verified" role={`Director · ${viewer.organizationName}`}>
      ...
      <section className={styles.workList} aria-labelledby="pending-title">
        <div className={styles.sectionHeading}><div><p>PENDING INVITATIONS</p><h2 id="pending-title">Access not yet accepted.</h2></div><span>{invitations.filter((invitation) => !invitation.accepted_at && !invitation.revoked_at && new Date(invitation.expires_at).getTime() > now).length} pending</span></div>
        {invitations.length === 0 && <section className={styles.emptyState}><h3>No invitations yet.</h3><span>Create one controlled staff invitation when team access is needed.</span></section>}
        {invitations.map((invitation) => {
          ...
          const state = invitation.accepted_at ? 'accepted' : invitation.revoked_at ? 'revoked' : new Date(invitation.expires_at).getTime() <= now ? 'expired' : 'pending';
          return <article className={styles.teamCard} key={invitation.id}><div><p>{state.toUpperCase()}</p><h3>{invitation.invited_email}</h3><dl className={styles.facts}><div><dt>Role</dt><dd>Staff</dd></div><div><dt>Location</dt><dd>{locationNames.join(' · ') || 'No location'}</dd></div><div><dt>Created</dt><dd>{formatOperationalTime(invitation.created_at)}</dd></div><div><dt>Expires</dt><dd>{formatOperationalTime(invitation.expires_at)}</dd></div><div><dt>Delivery</dt><dd>Not sent · manual handoff</dd></div><div><dt>Purpose</dt><dd>{invitation.purpose}</dd></div></dl></div>{state === 'pending' ? <RevokeInvitationForm invitationId={invitation.id} recipient={invitation.invited_email} /> : <p className={styles.formBoundary}>{state === 'accepted' ? 'Membership is active. Use the member access control below.' : state === 'revoked' ? 'No access was granted.' : 'This link can no longer be accepted.'}</p>}</article>;
        })}
      </section>
