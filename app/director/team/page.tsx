    ...
    <AppFrame active="team" identity={humanizePreviewIdentity(viewer.displayName, viewer.role)} mode="verified" role={`Director · ${humanizePreviewLabel(viewer.organizationName)}`}>
      ...
      <section className={styles.workList} aria-labelledby="members-title">
        ...
        {staffMembers.map((member) => {
          ...
          return <article className={styles.teamCard} key={member.id}><div><p>{humanMemberStatus(member.status)}</p><h3>{humanizePreviewIdentity(displayMember(member), member.role)}</h3><dl className={styles.facts}><div><dt>Role</dt><dd>Staff</dd></div><div><dt>Locations</dt><dd>{memberLocations.join(' · ') || 'No active location'}</dd></div><div><dt>Account</dt><dd>{member.user_id ? member.email : 'No sign-in account linked'}</dd></div><div><dt>Active commitments</dt><dd>{activeAssignments}</dd></div></dl></div>{member.status === 'active' && <RevokeMemberForm activeAssignmentCount={activeAssignments} memberId={member.id} memberName={humanizePreviewIdentity(displayMember(member), member.role)} requestId={randomUUID()} />}</article>;
        })}
      </section>
    </AppFrame>
  );
