...
export function InvitationForm({ creationRequestId, organizationName, locations }: Props) {
    ...
    return (
      <section className={styles.receipt} aria-live="polite">
        ...
        <dl>
          ...
          <div><dt>Visible to</dt><dd>Authorized directors, plus anyone holding the secure link until it expires or is revoked</dd></div>
          <div><dt>Acceptance boundary</dt><dd>Only the exact verified invited account can accept</dd></div>
          <div><dt>Proof saved to</dt><dd>Invitation record and append-only organization audit history</dd></div>
          <div><dt>Token proof</dt><dd>Ends in {receipt.tokenHint}</dd></div>
          <div><dt>Next action</dt><dd>{replayed ? 'Use the originally saved secure link. If it is unavailable, an isolated-lab administrator must run the guarded cleanup before creating a replacement.' : 'Open the secure link in the staff test session'}</dd></div>
        </dl>
  ...
  return (
    <form action={action} aria-busy={pending} className={styles.form}>
      ...
      <label>Verified staff email<input autoComplete="off" name="invitedEmail" placeholder="cycle7a-staff@passage.test" required type="email" /></label>
      ...
      <section className={styles.review} aria-label="Invitation review"><strong>Review before creating</strong><p>Organization: {organizationName}</p><p>Role: Staff · Delivery: Not sent</p><p>Proof: invitation record plus append-only creation event</p><p>Next state: pending acceptance by the exact verified email</p></section>
      ...
      <p className={styles.boundary}>Passage creates authority only through the server command. It will not send email, widen family access, or assign case work.</p>
    </form>
  );
}
