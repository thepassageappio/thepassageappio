...
// task.assigned / task.reassigned store a member id (or the literal string
// 'unassigned') in previous_state/next_state, not a status enum - see
// assign_task_idempotent in cycle_7b_assigned_work.sql. Resolve those two
// event types through the same member lookup used for "Changed by" instead
// of the status-label map.
function humanBeforeAfter(value: string | null, eventName: string, memberById: Map<string, Parameters<typeof displayMember>[0]>) {
  if (eventName === 'task.assigned' || eventName === 'task.reassigned') {
    if (!value) return 'Not recorded';
    if (value === 'unassigned') return 'Unassigned';
    return displayMember(memberById.get(value));
  }
