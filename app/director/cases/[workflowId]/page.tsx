...
export default async function DirectorCasePage({ params, searchParams }: { params: Promise<{ workflowId: string }>; searchParams: Promise<{ task?: string; vendorPayment?: string }> }) {
  ...
  return <AppFrame active="director" identity={humanizePreviewIdentity(viewer.displayName, viewer.role)} mode="verified" role={`${viewer.role === 'owner' ? 'Owner' : 'Director'} · ${humanizePreviewLabel(viewer.organizationName, 'Your organization')}`}>
    ...
    <section className={styles.panel} aria-labelledby="family-heading">
    ...
    <div className={styles.layout} style={{ marginTop: 18 }}>
