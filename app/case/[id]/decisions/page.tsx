...
export default async function FamilyCaseDecisionsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const result: FamilyCaseViewResult = await loadFamilyCaseView(id);
  if (!result.ok) {
    if (result.reason === 'signed-out') redirect(loginPath(`/case/${id}/decisions`));
    return <Closed reason={result.reason} />;
  }
  return <ComingSoonCaseSection caseId={id} label="Decisions" personName={result.data.workflow.personName} segment="decisions" />;
}
