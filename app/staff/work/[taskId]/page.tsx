...
import { formatPartnerAmount, formatPartnerTime, humanPartnerCategory, humanPartnerRequestStatus, loadPartnerRequestsForWorkflow } from '@/lib/partner/hosted';
...
import { BlockTaskForm, UnblockTaskForm } from './BlockTaskForm';
...
export default async function StaffWorkPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  const latestReviewer = latestReview ? humanizePreviewIdentity(displayMember(members.find((member) => member.id === latestReview.reviewed_by_organization_member_id)), 'director') : undefined;
  ...
  // Vendor status for this case (founder decision 2026-08-19): read-only,
  // scoped by RLS to cases this staff member is actually assigned work on.
  const partnerClient = await createPassageServerClient();
  const partnerRequests = partnerClient ? (await loadPartnerRequestsForWorkflow(partnerClient, workflow.id)).requests : [];
  ...
  return (
    <AppFrame active="staff" identity={humanizePreviewIdentity(viewer.displayName, viewer.role)} mode="verified" role={`Staff · ${humanizePreviewLabel(viewer.organizationName, 'Your organization')}`}>
      ...
      <ol aria-label="Case Room position" className={styles.orientation}><li aria-current="step" data-active="true">Now</li><li>Tasks</li><li>Proof</li></ol>
      ...
      <div className={styles.layout}>
        <section className={styles.panel} aria-labelledby="now-heading">
          ...
          <h2 id="now-heading">{task.status === 'completed' ? 'Verified. Task complete.' : task.status === 'proof_submitted' ? 'Waiting for director review.' : needsReplacement ? 'Submit replacement proof.' : task.status === 'blocked' ? 'Blocked. Clear it below once resolved.' : task.status === 'assigned' ? 'Start this work from My work.' : 'Complete the work, then save proof.'}</h2>
          ...
          {task.status === 'in_progress' && <BlockTaskForm requestId={randomUUID()} taskId={task.id} version={task.version} />}
          {task.status === 'blocked' && <UnblockTaskForm requestId={randomUUID()} taskId={task.id} version={task.version} />}
          ...
          {task.status === 'completed' && latestProof && <ProofReceipt proof={latestProof} reviewedAt={latestReview?.reviewed_at} reviewer={latestReviewer} submitter={humanizePreviewIdentity(displayMember(members.find((member) => member.id === latestProof.submitted_by_organization_member_id)), 'staff')} verified />}
        </section>
        <aside className={styles.panel} aria-labelledby="facts-heading"><p className={styles.eyebrow}>Task facts</p><h2 id="facts-heading">What this step carries.</h2><dl className={styles.facts}><div><dt>Owner</dt><dd>{ownerName}</dd></div><div><dt>Waiting party</dt><dd>{humanizePreviewLabel(task.waiting_party ?? '', 'Nobody recorded')}</dd></div><div><dt>Due</dt><dd>{formatOperationalTime(task.due_at)}</dd></div><div><dt>Audience</dt><dd>{humanAudience(task.audience)}</dd></div><div><dt>Passage prepared</dt><dd>{humanizePreviewLabel(task.prepared_output ?? '', 'No prepared output')}</dd></div><div><dt>What you do</dt><dd>{humanizePreviewLabel(task.human_action ?? '', 'Complete the assigned work')}</dd></div><div><dt>Proof is saved in</dt><dd>{humanizePreviewLabel(task.proof_destination ?? '', 'This task’s proof history')}</dd></div><div><dt>Next action</dt><dd>{task.status === 'blocked' ? 'Clear the blocker below once it’s resolved' : task.status === 'assigned' ? 'Return to My work and start the task' : 'Director reviews submitted proof'}</dd></div></dl></aside>
      </div>
      <section className={styles.panel} aria-labelledby="history-heading" style={{ marginTop: 18 }}><p className={styles.eyebrow}>Proof</p><h2 id="history-heading">Proof and replacement history.</h2>{taskProofs.length === 0 ? <p>No proof has been submitted.</p> : <ol className={styles.history}>{taskProofs.map((proof, index) => { const review = reviewByProof.get(proof.id); const reason = humanizeSavedReason(review?.reason ?? null, 'Please replace this proof with a clearer or corrected submission.'); const submitter = humanizePreviewIdentity(displayMember(members.find((member) => member.id === proof.submitted_by_organization_member_id)), 'staff'); const reviewer = review ? humanizePreviewIdentity(displayMember(members.find((member) => member.id === review.reviewed_by_organization_member_id)), 'director') : null; const replacedIndex = proof.supersedes_proof_id ? taskProofs.findIndex((candidate) => candidate.id === proof.supersedes_proof_id) : -1; const replacedProof = replacedIndex >= 0 ? taskProofs[replacedIndex] : null; return <li key={proof.id}><h3>{review?.decision === 'verified' ? 'Proof verified, task complete' : review?.decision === 'needs_replacement' ? 'Replacement requested' : 'Proof submitted for review'}</h3><p>{humanizePreviewLabel(proof.completion_summary, 'Proof summary available')}</p>{proof.reference && <p>Supporting reference: {humanizePreviewLabel(proof.reference, 'Reference saved')}</p>}<small>Evidence #{index + 1} · {humanProofType(proof.proof_type)} · submitted by {submitter} · {formatOperationalTime(proof.submitted_at)}</small>{replacedProof && <small>Replaces evidence #{replacedIndex + 1} submitted {formatOperationalTime(replacedProof.submitted_at)}</small>}{reason && <p>Director reason: {reason}</p>}{review && <small>Director decision: {review.decision === 'verified' ? 'Verified' : 'Replacement requested'} by {reviewer} · {formatOperationalTime(review.reviewed_at)}</small>}</li>; })}</ol>}</section>
      <section className={styles.panel} aria-labelledby="vendor-heading" style={{ marginTop: 18 }}>
        ...
        {partnerRequests.length === 0 ? <p>No vendor requests have been sent for this case yet.</p> : (
...
function ProofReceipt({ proof, reviewedAt, reviewer, submitter, verified = false }: { proof: { completion_summary: string; proof_type: string; reference: string | null; submitted_at: string }; reviewedAt?: string; reviewer?: string; submitter: string; verified?: boolean }) {
  return <div className={styles.receipt} role="status"><h3>{verified ? 'Verified. Task complete.' : 'Waiting for director review.'}</h3><p>{humanizePreviewLabel(proof.completion_summary, 'Proof summary available')}</p>{proof.reference && <p>Supporting reference: {humanizePreviewLabel(proof.reference, 'Reference saved')}</p>}<small>{humanProofType(proof.proof_type)} · submitted by {submitter} · {formatOperationalTime(proof.submitted_at)} · the current task owner and directors authorized for this location can see it</small>{verified && reviewer && reviewedAt && <small>Director decision: Verified by {reviewer} · {formatOperationalTime(reviewedAt)}</small>}</div>;
}
