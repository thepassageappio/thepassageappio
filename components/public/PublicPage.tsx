...
type Card = { title: string; body: string; href?: string; linkLabel?: string; availabilityLabel?: string };
    ...
    <div className={styles.grid}>
      {cards.map((card) => (
        <article className={styles.card} key={card.title}>
          ...
          <p>{card.body}</p>
          {card.href
            ? <Link href={card.href}>{card.linkLabel ?? 'Read more'} <span aria-hidden="true">→</span></Link>
            : card.availabilityLabel && <span className={styles.availability}>{card.availabilityLabel}</span>}
        </article>
      ))}
    </div>
  );
