...
const ROLE_TEMPLATES = [
 {
  id: "executor", label: "Executor / Estate Attorney", icon: "",
 ...
 {
  id: "funeral", label: "Funeral Home / Director", icon: "",
...
const GoogleSignInBtn = ({ label = "Continuewith Google", compact = false }) => (
...
function EventsModal({ workflowId, deceasedName, onClose, onSaved }) {
 ...
 const EVENT_TYPES = [
  { type: 'visitation', label: 'Visitation / Wake / Shiva', icon: '' },
  { type: 'funeral', label: 'Funeral / Memorial Service', icon: '' },
  { type: 'burial', label: 'Burial / Committal', icon: '' },
     ...
     <div>
      ...
      <div style={{ marginBottom: 14 }}>
       ...
       <div style={{ display: "flex", gap: 7 }}>
        {[["both","Email + SMS"],["email","Email only"],["sms","SMS only"]].map(([v, l]) => (
      ...
      <div key={tier} style={{ marginBottom: 9 }}>
       <button onClick={() => setExpanded(p => ({...p, [tier]: !p[tier] }))}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: meta.bg, borderRadius: isOpen ? "13px 13px 0 0": 13, padding: "11px 14px", border: `1px solid ${meta.color}22` }}>
         ...
         <span style={{ fontSize: 13, color: meta.color }}>{isOpen ? "Collapse": "Expand"}</span>
        </div>
       </button>
...
function PlanFlow({ onComplete, onBack, user, onSignOut, onDashboard }) {
 ...
 const activate = async (mode) => {
  ...
  if (!user?.id) {
   setPlanError("Sign in once so Passage can save this plan to your family record.");
   setStep(0);
   return;
  }
  setPlanError("");
  ...
  try {
   ...
   if (!accessToken) throw new Error("Sign in once so Passage can save this plan to your family record.");
  ...
  <Card key={0} maxWidth={920}>
   <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.35fr) minmax(260px,.65fr)", gap: 16, alignItems: "start", marginBottom: 16 }}>
    <div>
     ...
     <Sub>This will make things easier for your family later. Build the family record before it is needed: people, wishes, documents, and trusted contacts.</Sub>
    </div>
...
function Dashboard({ user, onStartPlan, onEmergency, onSignOut, onOpenPlan, onHome, onDashboard, refreshKey }) {
 ...
 const [selectedDashboardEstateId, setSelectedDashboardEstateId] = useState("");
 ...
 useEffect(() => {
  if (!activeWorkflows.length) {
   if (selectedDashboardEstateId) setSelectedDashboardEstateId("");
   return;
  }
  if (!selectedDashboardEstateId || !activeWorkflows.some(wf => String(wf.id) === String(selectedDashboardEstateId))) {
   setSelectedDashboardEstateId(String((redWorkflows[0] || greenWorkflows[0] || activeWorkflows[0]).id || ""));
  }
 }, [activeWorkflows.length, selectedDashboardEstateId]);
 ...
 const selectedDashboardEstate = activeWorkflows.find(wf => String(wf.id) === String(selectedDashboardEstateId))
  || redWorkflows[0]
  || greenWorkflows[0]
  || activeWorkflows[0]
  || null;
 const selectedDashboardStats = selectedDashboardEstate ? (taskStatsByWorkflow[selectedDashboardEstate.id] || {}): {};
 const selectedDashboardOrchestration = selectedDashboardEstate ? estateOrchestrationByWorkflow.get(selectedDashboardEstate.id): null;
 const selectedDashboardTask = selectedDashboardOrchestration?.nextTask || null;
 const selectedDashboardWorkspace = selectedDashboardTask ? taskWorkspaceFor(selectedDashboardTask, {
  persona: 'family',
  estateName: selectedDashboardEstate?.name || 'this estate',
     ...
     <>
       ...
       <div style={{ background: C.bgCard, borderRadius: 18, padding: "18px", border: `1px solid ${C.border}`, marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 220px), 1fr))", gap: 10, marginTop: 14 }}>
         ...
         <button onClick={() => window.location.href = '/urgent'} style={{ textAlign: "left", background: C.roseFaint, border: `1px solid ${C.rose}30`, borderRadius: 13, padding: "14px", cursor: "pointer", fontFamily: "inherit" }}>
          ...
          <div style={{ fontSize: 12.5, color: C.mid, lineHeight: 1.5 }}>Start urgent help first. Passage will create the family record after the first-step plan.</div>
      ...
      <div style={{ background: C.bgCard, borderRadius: 18, padding: "16px", border: `1px solid ${C.border}`, marginBottom: 12 }}>
        ...
        <button type="button" onClick={() => onOpenPlan(selectedDashboardEstate, selectedDashboardTask.id, selectedDashboardTask.assignedTo ? 'open': 'assign')} style={{ width: "100%", textAlign: "left", background: selectedDashboardEstate?.path === 'green' ? C.sageFaint: C.roseFaint, border: `1px solid ${selectedDashboardEstate?.path === 'green' ? C.sageLight: C.rose + '30'}`, borderLeft: `5px solid ${selectedDashboardEstate?.path === 'green' ? C.sage: C.rose}`, borderRadius: 14, padding: "14px", cursor: "pointer", fontFamily: "inherit" }}>
         <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}>
          <div>
           ...
           <div style={{ marginTop: 8, background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 11, padding: "9px 10px", fontSize: 12, color: C.mid, lineHeight: 1.45 }}>
          ...
          <span style={{ fontSize: 11.5, fontWeight: 900, color: selectedDashboardTask.assignedTo ? C.sage: C.rose, whiteSpace: "nowrap", alignSelf: "center" }}>
           {selectedDashboardTask.assignedTo ? "Open ->": "Assign ->"}
          </span>
         </div>
        </button>
       ): (
       ...
       <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
        <button onClick={() => onOpenPlan(selectedDashboardEstate)} style={{ border: "none", borderRadius: 11, padding: "10px 13px", background: C.sage, color: "#fff", fontFamily: "inherit", fontWeight: 900, cursor: "pointer", fontSize: 12.5 }}>{selectedDashboardEstate?.path === 'green' ? 'Open planning record': 'Continuethis estate'}</button>
       </div>
      </div>
      <details style={{ background: C.bgCard, borderRadius: 14, padding: 0, border: `1px solid ${C.border}`, marginBottom: 12 }}>
       <summary style={{ cursor: "pointer", listStyle: "none", padding: "12px 14px", fontFamily: "Georgia, serif", fontSize: 14, color: C.mid, display: "flex", justifyContent: "space-between", gap: 10 }}>
        <span>All estate lists and plan details</span>
        <span style={{ color: C.sage, fontWeight: 800 }}>{activeWorkflows.length} estate{activeWorkflows.length === 1 ? '': 's'}</span>
       </summary>
      ...
      {false && redWorkflows.length > 0 && (
       <div style={{ background: C.bgCard, borderRadius: 18, padding: "18px", border: `1px solid ${C.border}`, marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
         <div style={{ fontFamily: "Georgia, serif", fontSize: 17, color: C.ink }}>Active family records</div>
      ...
      {/* Planning estate file */}
      {false && hasAnyEstate && estateSeatLimit > 0 && (
   ...
   {showMemoriesInfo ? (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setShowMemoriesInfo(false)}>
     <div onClick={e => e.stopPropagation()} style={{ background: C.bgCard, borderRadius: "20px 20px 0 0", padding: "28px 20px 48px", width: "100%", maxWidth: 560 }}>
      ...
      <div style={{ fontSize: 13, color: C.mid, lineHeight: 1.7, marginBottom: 20 }}>
       Record voice notes, write letters to loved ones, and preserve memories to be delivered at the right moment: a birthday, graduation, or simply when someone needs to hear from you.
      </div>
      <div style={{ background: C.goldFaint, border: "1px solid " + C.gold + "30", borderRadius: 11, padding: "12px 14px", fontSize: 13, color: C.amber, marginBottom: 20, lineHeight: 1.55 }}>
  ...
  <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Georgia, serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
   <div style={{ width: '100%', maxWidth: isGreenPlan ? 660: 560, background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 20, padding: '28px 24px', textAlign: 'center', boxShadow: '0 18px 50px rgba(55,45,35,.08)' }}>
    ...
    <div style={{ fontSize: 14, color: C.mid, lineHeight: 1.7, marginBottom: 20 }}>
     {isPreview
      ? 'Open the family record to handle next steps, assign owners, and see what comes next.'
    ...
    {onDashboard ? (
     <>
      <button onClick={() => {
       ...
       {isGreenPlan ? 'View your plan': 'Continueto My file'}
      </button>
      {isGreenPlan && (
