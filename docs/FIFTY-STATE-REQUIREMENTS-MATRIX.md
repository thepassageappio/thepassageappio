...
## C–G. Master table and catalogs

Full §§C–G content (50-state master table, encode dimension catalog, false-claim guardrails, research gaps, counts) is in the companion file [`FIFTY-STATE-REQUIREMENTS-MATRIX-TABLE.md`](./FIFTY-STATE-REQUIREMENTS-MATRIX-TABLE.md) — same 2026-09-15 research source as `/workspace/compliance-research/FIFTY-STATE-REQUIREMENTS-MATRIX.md`. Combined single-file landing preferred; companion split used when MCP payload limits prevent one-shot 33KB push.
