...
- **Deployed and smoke-tested live** (Vercel build `dpl_9GaEAbJX4szeSCNAzK396Bz81X2v`, clean build, no new errors). Anonymous load of a real case's `/case/[id]/tasks` URL redirects correctly to sign-in with no crash and no console errors. **Not yet verified: an authenticated family owner actually seeing a real vendor-request item rendered on the page**, because there are currently zero real `partner_requests` rows attached to any real `workflows` row in production (confirmed by direct query) -- the adversarial SQL test used synthetic fixtures, not live data. This should be re-checked in the browser the first time a real vendor request is created against a real case.

**L.2-L.5, still open:**
- **L.2** -- a family/D2C self-service task-completion RPC (the security-sensitive non-staff-owner-acting-on-own-case capability flagged as blocking further D2C checklist work back in Phase K).
- **L.3** -- the draft-and-send communication engine: Resend SDK wiring (`RESEND_API_KEY` needs to be added to Vercel by the founder), a new `task_communications` table using the doctrine's exact prepared/reviewed/sent/delivered vocabulary, and prepare/send RPCs supporting both platform-known recipients and ad-hoc external recipients (e.g. "email everyone about the service, wake, and burial").
- **L.4** -- seed the already-researched urgent and planning-ahead D2C checklists, unblocked once L.2 exists.
- **L.5** -- Decisions, Service, and Costs family pages (Costs needs its own billing/estimate data-model design pass, not bundled here).
