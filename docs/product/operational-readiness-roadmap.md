...
| 5 | Named recovery owner for every failure | ✅ `crm_sync_events` logs every HubSpot Deal-creation failure across all 4 Stripe-webhook sites plus `organization/start`'s trial-deal creation (2026-08-18: closed via `passage_private.log_crm_sync_failure`, a `SECURITY DEFINER` RPC, since that site runs on the user-context client and `crm_sync_events`' RLS has zero policies — a direct authenticated insert would be silently blocked, and `createPassageServiceClient()` is explicitly documented as forbidden outside trusted service contexts) |
...
Fixed in `supabase/migrations/20260818070000_fix_missing_family_participant_rpcs.sql`, applied to production, verified with a zero-footprint adversarial SQL test (real participant gets a real row; unrelated user gets zero rows; director's grant-toggle works via the new public wrapper), committed and pushed (`986e02f`).

**PR #77 reconciled and closed (2026-08-18).** It independently diagnosed and fixed the same participant-lockout symptom back on 2026-08-10, but its entire fix (`get_family_case_update_for_workflow`, both migrations, both `lib/family/*-view.ts` changes) was built against `continuity_spaces`/`continuity_participants` — confirmed via direct production query to not exist in `qsveqfchwylsbncsfgxe` at all, ever. That PR was branched off `release/10h-delivery`, a separate release line from `main` where that lab-only system was apparently still live; it was deliberately never ported to production. Tonight's migration is the only valid production fix — this is now fully closed, not just probably-superseded.

### Structured participant roles — shipped 2026-08-18, plus a critical pre-existing bug found and fixed in the same pass

`docs/product/persona-action-architecture.md` (persona table, lines 173-184) already lists **Executor/estate administrator**, **Celebrant/venue/clergy**, **Cemetery/crematory**, and **Vendor** as distinct personas — correctly scoped in docs, not built as data. Founder decision (asked directly): Executor and POA/medical-proxy get real differentiated authority — task creation/management and inviting other participants, scoped to only their one case (not cross-estate); clergy/officiant and cemetery/crematory contact stay display-label only, same baseline as a plain participant.

**Shipped**: `case_family_invitations.participant_role` (5 values), `estate_access.role` gained `authorized_participant`, `accept_case_family_invitation` now derives it from the invitation instead of hard-coding `'participant'`, `can_invite_family_to_workflow` extended to cover an elevated participant on their own case (which also extends `revoke_case_family_invitation` for free, since it shares the same predicate), new `create_family_task_idempotent` + `list_family_tasks_for_participant` RPCs (D2C-only, mirroring the org-side `create_task_idempotent` shape but keyed by `workflow_id` since D2C cases have no `organization_id`), `set_family_task_completion_idempotent` widened to also accept an elevated participant. Migration `supabase/migrations/20260818110000_family_participant_structured_roles.sql`. 15/15 adversarial SQL assertions passed (owner-only vs. elevated vs. plain-participant vs. unrelated-user authority, cross-case isolation, idempotent replay on both invite-creation and task-creation, elevated-only task completion, plain participants get read-only task visibility with a null completion version).

**CRITICAL, found via this session's own adversarial testing, not a spot check: `accept_case_family_invitation` has never successfully executed in production, for any invitation, ever.** Its `on conflict (workflow_id, user_id)` target list is ambiguous against the function's own `RETURNS TABLE(workflow_id uuid, ...)` output columns (PL/pgSQL registers `RETURNS TABLE` columns as implicit variables; Postgres raises 42702 "ambiguous" the moment a bare column name in the SQL body collides with one). Confirmed empirically before fixing: zero rows in `case_family_invitations` have ever had `accepted_at` set, and the single `estate_access` row that exists in production (`role='external_partner'`) was not created through this path at all. This predates today's `participant_role` work entirely — it's the original, always-broken accept flow, meaning **every family-invite acceptance attempt by every real user has always failed**, silently, this entire time. Fixed same session (`#variable_conflict use_column` pragma, applied directly to `accept_case_family_invitation` — safe because every actual read of the `RETURNS TABLE` columns in the function body is already table-qualified, never bare) and reverified with the same 15-assertion suite, all passing including the accept step itself. **This is the single most consequential bug found this entire project** — bigger than the RPC-public-wrapper fixes from earlier today, since those only blocked *viewing* a case after acceptance; this one blocked *acceptance itself*, meaning the family-invite feature has been completely non-functional end-to-end since it shipped, with zero prior detection.

**Still open, not yet built this session:** UI wiring — role dropdown on the two existing invite-creation forms (`app/director/cases/[workflowId]/FamilyInvitationForm.tsx`, `app/case/EstateActions.tsx`'s `InviteToEstateForm`), display of `participant_role` everywhere `relationship` is shown, and a new participant-facing page (no existing page is reachable by an accepted participant to host a task-create/invite form — `/case/page.tsx` is hard-scoped to `workflows.user_id = auth.uid()`, participants never reach it).
