...
9. A distinct Independent Agent Reviewer passes the exact reviewed head, then a distinct Development Head / Release Authority approves the same head through `Passage Review Agent / merge-review`. That authority must not be the author/implementer, QA, Independent Agent Reviewer, or Deploy role. A new head, base, evidence set, or required-check change makes the approval stale. Production separately requires owner authorization through the protected Production environment or release gate for the exact commit.
