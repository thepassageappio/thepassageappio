...
9. A distinct Independent Agent Reviewer passes the exact reviewed head, then a distinct Development Head / Release Authority approves the same head through `Passage Review Agent / merge-review`. That authority must not be the author/implementer, QA, Independent Agent Reviewer, or Deploy role. A new head, base, evidence set, or required-check change makes the approval stale. Reversible Production promotion separately requires exact-head authorization from the distinct Production Reviewer; owner involvement is limited to destructive Production data, spending, or material legal/privacy/security judgment.
