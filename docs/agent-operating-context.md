...
- Founder screenshots then exposed the adjacent green-path failure: `Continue planning` opened the funeral-care Today projection, rendered `Status unavailable`, claimed a nonexistent care team owned the work, and provided no action for the visible planning task. The same care-team language leaked into checklist and message pages.
- Engineering now marks true green, organization-free workflows as planning records in the family projection; normalizes historical task statuses; renders a planning-specific overview with a real checklist action, private-plan visibility, and saved activity; uses planning-specific Overview / Checklist / Share navigation; labels self-serve checklist ownership as `You`; and removes care-team copy from green sharing. Funeral-home-backed family cases retain their existing projection.
- Structured wishes/preferences capture remains **not implemented**: the production schema has only actionable planning tasks, not a durable preferences model. This repair does not pretend that checking off `Document your funeral and disposition preferences` saved structured wishes. A later schema slice requires a documented field model, migration, RLS, event proof, recovery, and explicit Production authorization before application.
