# Threshold hydration P1 mobile recovery candidate - 2026-07-21 19:24 -07:00

- Lane and protected base: separate Threshold/main Production-maintenance P1. PR #31 exact source head `dda5963e664127ee965d0b4921c71daa536b3e86` passed its Preview and exact-head checks, received exact owner Production authorization, merged as protected main `09ddb9e5601432f9dd2c36bbdbd829719dd66859`, and deployed as `dpl_5GVpgdmZ6oqLkVeWcgcrwSpGPNmj`. Passage Zero, PR #24/#30, isolated Supabase, readiness scores, pricing, family/vendor scope, and Production database state were untouched.
- Post-deploy finding and rollback: real Chrome at the 360 target produced an actual 345px content width and found `/mission` horizontal overflow (`357 > 345`). Deploy therefore failed the post-deploy gate and used Vercel Instant Rollback to restore the pinned prior READY Production deployment `dpl_3rAyuahrHAqcoH5KJLykL6mR2JSR`. The failed release is marked as the rollback candidate. Restored Production retains its already-known hydration errors; that truth is why this P1 recovery remains FIX NOW rather than being closed.
- Product Manager re-entry `/root/pm_next_impactful_block`: PM Sprint Brief COMPLETE for one bounded repair. Preserve the reviewed desktop composition; collapse only the three `/mission` grids at the existing mobile breakpoint; do not change copy, IA, navigation, pricing, auth, motion, fonts, other routes, Passage Zero, or readiness. Acceptance is width equality and no genuine overflow at 1440/390/360, 44px path controls, unchanged reduced motion/focus behavior, zero hydration/console/page/runtime errors, then the full six-route and comparison-route regression.
- UI/UX Review re-entry `/root/ux_threshold_hydration_p1`: PASS TO ENGINEERING WITH CONDITIONS. Keep the desktop hero/proof/path columns unchanged; at `max-width: 720px`, stack each as one `minmax(0, 1fr)` column in existing DOM order. No JavaScript viewport detection, global overflow hiding, copy change, or behavioral drift.
- Development Engineering `/root/engineering_threshold_hydration_finish`: COMPLETE. Only `pages/mission.js` changed relative to the PR #31 source: three scoped grid classes preserve the exact desktop values and share the mobile one-column rule; `minWidth: 0` is limited to the two intrinsic hero items and active path content item. Static JSX parse PASS.
- Dedicated code review `/root/review_pr30_partial_closeout`: APPROVE / PASS TO QA with no blocking or nonblocking findings. The review independently confirmed exact desktop parity, scoped mobile reflow, no global overflow hiding or viewport JavaScript, and no copy, IA, interaction, reduced-motion, or visual-system drift.
- Independent QA re-entry `/root/qa_threshold_hydration_p1`: LOCAL/SOURCE PASS. Optimized Next 14.2.35 build generated 69/69 pages. System Chrome against the production server passed `/mission` at 1440x900, 390x844, and 360x800: HTTP 200, `clientWidth === scrollWidth === bodyScrollWidth`, no genuine overflow offender, all three path buttons exactly 44px high, and zero console, page, hydration, or runtime errors. The clipped empty 1x1 accessibility shim at left `-1` does not contribute scroll width and was not treated as layout overflow. The complete 10-route x 3-viewport deterministic regression also passed with no hydration, page, console, overflow, blank-page, font, query-return, or scoped target-size failure. Timestamped local screenshots are held in the excluded review-evidence workspace.
- Deploy status: PARTIAL. No new branch, pull request, Preview, merge, or Production deployment exists for this recovery yet. The next safe action is one narrow Passage Release Bot-authored draft from exact protected main, followed by a truthful non-production Preview path that does not mark the merge candidate `[qa-approved]` before hosted PASS. Hosted 1440/390/360 proof and fresh exact-head QA/Review checks remain required before any merge decision.
- Owner gates: none for the Bot-authored draft or non-production Preview. A later Production promotion remains separately gated for the exact release commit after hosted PASS. The earlier PR #31 authorization does not silently authorize a different recovery head.
- Auto-advance: post-deploy FAIL returned Deploy -> PM -> UX -> Engineering -> dedicated Review -> independent local QA. The train is advancing to Bot publication and hosted Preview verification; it is not stopping at source PASS.
...
- Files changed: `AGENTS.md`, `docs/release-train.md`, `docs/product/release-governance-and-plain-language-policy.md`, the Development Head role brief/legacy control alias, PR template, context checker, and this handoff. No runtime, database, environment, deployment, pricing, family/vendor, or readiness state changed.
...
- Next action: Bot-author the bounded docs-only pull request; distinct Independent QA and Development Head review the exact head; then resume the revenue-ranked Passage Zero roadmap without asking the owner for routine approvals.
