...
### Hosted family-pass hydration recovery — exact-head P1 repair — 2026-07-27

- Independent Agent Reviewer `/root/review_demo_expiry_cddd53a` rejected exact Bot-authored commit `cddd53a41567c328720f6289267089eaadf66a6c` before push. The commit identity, clean tree, marker discipline, and main hydration repair were correct, but JavaScript's permissive `Date.parse` accepted parseable noncanonical values: `0` became a 2000 instant, `2026-02-30` rolled into March, and date-only/local/offset values were treated as saved active state. This contradicted the PM and QA claim that malformed storage fails closed.
- Product Manager disposition remains `FIX NOW` within the frozen recovery brief; no new scope or owner gate applies. Development Engineer `/root` added exact canonical-instant validation: only `YYYY-MM-DDTHH:mm:ss.sssZ` values that round-trip byte-for-byte through `toISOString()` may derive, normalize, or format a deadline. Invalid JSON is removed from session storage. Regression coverage now rejects numeric-looking, impossible-calendar, date-only, offset/local, and missing-millisecond timestamps in both the parser and saved-draft path.
- Fresh Independent QA `/root/qa_demo_expiry_finalizer` rejected the first uncommitted P1 repair after all other gates and a clean 29/29 build passed. It proved that a valid four-digit activation near year 9999 could derive a `+010000` expiration outside the canonical contract, and that calling `sessionStorage.removeItem` inside the recovery catch could rethrow when storage itself was denied. Development now canonical-validates the derived result and catches storage cleanup independently, preserving the honest in-memory example even when browser storage is unavailable. Regression coverage includes the year-boundary derivation and storage-denial recovery binding.
- `cddd53a` remains preserved as rejected evidence and will not be amended or pushed. Source QA **NOT RUN after this P1 repair**; Hosted Preview QA **NOT RUN for a replacement**; Production Deployment **NOT DEPLOYED**; Production QA **NOT RUN**; Overall release state **SOURCE REPAIRED / PREVIEW BLOCKED / NOT QA-APPROVED**. Exact next role: fresh Independent Source QA on the uncommitted repair, then a new Bot commit and fresh exact-head review chain if PASS.
