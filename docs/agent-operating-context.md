...
- Repository branch protection/rules must require pull requests and the trusted governance check, require one non-author approval, dismiss stale approvals, resolve conversations, restrict bypass, and block force-push/deletion. That external configuration is the direct-main prevention mechanism and remains an explicit gate, not a source-file claim.
...
- Product Manager re-scoped all three issues FIX NOW. The corrected head `657f21e9c175adac983261eadd3a4a72ecd1c350` removes the push event, pins candidate checkout by full SHA with no persisted credentials or review token, and evaluates release/human-review gates only through base-defined inline logic in the read-only `pull_request_target` workflow. That trusted workflow never checks out or executes candidate code.
- Independent QA `/root/qa_governance_language`: **PASS** on exact PR #25 head `657f21e9c175adac983261eadd3a4a72ecd1c350`. The 8-file governance-only diff is clean; governance fixtures, persona scan, legacy-bridge behavior, current-head non-author/non-bot human rule, and both GitHub workflows passed. This is agent QA, not human approval.
...
- PR #25 remains open, draft, unmerged, and has zero human reviews. Direct-main protection is not complete until a separate human reviews the current head, the bootstrap merges, and live GitHub rules require pull requests, the immutable check, one current-head human approval, stale-approval dismissal, resolved conversations, restricted bypass, and blocked force pushes/deletion.
- Cycle 8 remains FAIL/PARTIAL and uncommitted. No migration was applied. The next permitted action is to preserve these corrections in one `[skip deploy]` context/workflow commit, independently check the exact diff, and keep PR #25/Production blocked at the human-review gate.
