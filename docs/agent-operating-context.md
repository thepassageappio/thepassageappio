...
### Browser family A16 and operator sign-in fallback - Development Engineering handoff - 2026-08-13 00:02 -07:00

- **Role instance and received handoff:** distinct Development Engineer `/root/pm_combined_demo_a16_return/eng_demo_family_a16_return` received the complete PM Sprint Brief above on exact PM-only head `5c19113a504799e8bde9bd2461e1096adf77d8c1`. Exact lineage was verified before editing: `5c19113...` is a clean descendant of combined candidate `0065bd41c8191a0c8bed1dc60528d76c2d6ac5a5`, whose tree is `aa8117115e4863c2d59234e2992780e86485a069`. The later PM addendum required every configured Director, Staff, or Vendor sign-in failure to open the existing truthful guided browser demo while preserving successful isolated sign-in targets.
- **A16 route-composition repair:** `/demo/family` now explicitly passes `providerMode="browser_demo"` through `FamilyIntentJourney` and `TransferComposer` to the shared `FuneralHomeDiscovery`. Browser-demo search calls the existing pure `rankSyntheticProviders` function directly, and explicit directory or manual confirmation creates only a typed page-local selection. It contains no server replay field, family-space claim, durable-history claim, or external contact claim. The receipt states that the example choice exists only on the current page and clears on refresh or exit.
- **Authenticated behavior preserved:** `/family` explicitly passes `providerMode="authenticated"` and the existing `confirmProviderSelection` Server Action. The shared client renderer no longer statically imports that action. Selection preload, authenticated search, durable confirmation, RLS/authority, append-only receipt, replay/conflict handling, and fail-closed recovery remain in their existing authenticated branches. No failure, missing session, URL, pathname, hostname, or environment value can activate browser-demo mode.
- **Operator recovery addendum:** `startPreviewDemo` retains configured success destinations `/director`, `/staff`, and `/partner`. Client creation exceptions or absence, initial sign-out exception/error, sign-in exception/error including `invalid_credentials`, and identity mismatch now move to the matching existing `/demo/operator/{persona}` page. Post-attempt failures use best-effort sign-out first. The guided pages already say that no server session opened, no record changed, state exists only on the page, and refresh resets it; their navigation remains contained under `/demo`.
- **Permanent regression coverage:** `scripts/test-family-provider-discovery.mjs` binds explicit route composition, zero authenticated action import/pass in the demo graph, pure deterministic local ranking, authenticated action preservation, and no inferred/failure fallback. New `scripts/test-operator-demo-fallback.mjs` executes `invalid_credentials` for Director, Staff, and Vendor, proves best-effort cleanup, and proves each configured successful identity still redirects to its protected persona route. The public conversion guard binds every source failure to the guided fallback and prohibits the old sign-out, sign-in, and identity dead ends.
- **Files changed:** `app/demo/actions.ts`, `app/demo/family/page.tsx`, `app/family/page.tsx`, `components/family/FamilyIntentJourney.tsx`, `components/family/TransferComposer.tsx`, `components/family/provider-discovery/FuneralHomeDiscovery.tsx`, `lib/provider-discovery/types.ts`, `package.json`, `scripts/test-family-provider-discovery.mjs`, `scripts/test-public-conversion.js`, `scripts/test-operator-demo-fallback.mjs`, and this living context. No schema, migration, RLS, RPC, parity ledger, roadmap, fixture, environment, credentials, Supabase resource, Vercel configuration, pricing, or Production resource changed.
- **Engineering source verification:** provider discovery PASS; public conversion PASS; operator configured-sign-in fallback PASS; Participant invitation P1/P2 source guard `42/42` PASS; workflow messaging `27/27` PASS; participant case authority `25` PASS; frontend/backend parity `22/22` PASS; Server Action export guard PASS with two Cycle 8, nine invitation, and one A16 action; persona language PASS; context PASS before this append; release governance PASS; operational routes PASS; runtime isolation PASS; deploy-decision matrix PASS; TypeScript PASS; optimized Next.js 16.1.6 build PASS with 51 routes; React/Next quality review PASS; and `git diff --check` PASS. The optimized build emitted the known multiple-lockfile warning and a trailing Windows access-denied noise line after successful exit `0`; compilation, TypeScript, static generation, and route inventory all completed.
- **Environment-bound evidence:** the Participant P2 race runner was NOT RUN in this source worktree because `PASSAGE_P2_RACE_PROJECT_REF` and its isolated credentials are not present. It failed closed before any connection. The unchanged P2 source/race contract remained green in the `42/42` guard. Fresh exact-head QA owns any credential-bound rerun against isolated `uyacxqtsiwlvtmhxvoxr` only.
- **Local browser evidence and infrastructure fix-it:** browser QA is **NOT RUN**. A local development server attempt failed to bind. The optimized production server later bound port `3217`, but both a direct HEAD request and in-app browser navigation stalled without returning a page. Watchdog terminated only the known local server process and finalized the browser tab. Named fix-it owner: fresh Independent QA. Affected cells: New York suggestions, explicit selection, Receiver to Access to Timing to Review to pass, manual entry, zero provider-endpoint network capture, responsive/focus/runtime cells, and the three forced operator-failure navigations. Recovery test: start the exact committed build in a clean runner, capture all requests, prove zero `/family/provider-discovery/search` and `/family/provider-discovery/selection` requests on `/demo/family`, complete both directory and manual flows, and prove each operator failure lands on the matching guided page with no protected navigation.
- **Roadmap material-change classification:** **NO**. This restores already-required deterministic demo and truthful recovery behavior. It changes no product direction, scope, milestone order, persona coverage, architecture, readiness doctrine, or score.
- **Source QA:** ENGINEERING GATES PASS / DISTINCT EXACT-HEAD SOURCE AND DATA QA NOT RUN.
- **Hosted Preview QA:** PRESERVED FAIL on original combined Preview cell; replacement descendant Preview NOT DEPLOYED and QA NOT RUN.
- **Production Deployment:** NOT DEPLOYED.
- **Production QA:** NOT RUN.
- **Overall release state:** SOURCE REPAIR COMPLETE / BROWSER QA NOT RUN / NON-PRODUCTION / NOT QA-APPROVED.
- **Auto-advance target:** freeze one Passage Release Bot `[skip deploy]` descendant and immediately hand the exact head/tree to fresh UX re-review and Independent QA. QA must rerun source/build checks and own the named browser recovery cells before any replacement Preview. No owner prompt is required.

### PM addendum - configured operator sign-in safe fallback - 2026-08-12 23:36 -07:00

- **New evidence and classification:** credential recovery stopped safely. The official Auth Admin request failed at the network boundary, the browser password form was correctly prohibited, and no Auth, database, environment, or Production mutation occurred. Configured credentials can therefore be present while `signInWithPassword` still returns `invalid_credentials`. The current redirect back to `/demo?demo=signin` leaves Director, Staff, and Vendor without the already-built guided fallback. Classify this as **FIX NOW** in the same demo repair descendant. Credential drift remains separate Deploy configuration debt; this source repair does not hide or resolve it.
- **Frozen behavior:** when a persona's configured operator session signs in and the exact identity is verified, preserve the current real server-session destination: Director `/director`, Staff `/staff`, Vendor `/partner`. When configured sign-in cannot safely complete, automatically open that persona's existing guided browser-local route: `/demo/operator/director`, `/demo/operator/staff`, or `/demo/operator/vendor`.
- **Failure set:** the guided fallback applies after a server-client creation exception/unavailability, sign-out failure, `signInWithPassword` exception, Auth error including `invalid_credentials`, missing user, or exact-email mismatch. Perform best-effort sign-out whenever a client/session may exist. Do not loop through the gateway, display a hidden query failure, retain a partial session, navigate to a protected workspace, or expose the failure reason, email, credential state, project, or infrastructure detail.
- **Truth contract:** the guided destination must visibly say that no server session opened, no record changed, the example state exists only on this page, and refresh resets it. Existing route-contained shell/footer navigation remains mandatory. It must not claim durable, shared, authenticated, server-authorized, sent, or cross-persona state. This fallback is a usable browser example, not recovery evidence for the credential/config debt.
- **Source acceptance:** deterministic tests cover Director, Staff, and Vendor `invalid_credentials` and prove each maps to its exact `/demo/operator/{persona}` route after best-effort cleanup. Additional tests cover thrown sign-in, missing/wrong identity, and sign-out/client failure without protected-route navigation. The configured-success source contract still proves exact verified-email success maps only to `/director`, `/staff`, or `/partner`. Unconfigured personas continue to take the same guided route directly.
- **Hosted acceptance:** on the exact descendant Preview, each failing configured persona reaches its correct guided page in one action; final URL, page-only/reset copy, contained navigation, actions, focus, reload reset, console/page errors/rejections/failed requests, and 1440/390/360 behavior are captured. A separately repaired credential later proves the real server-session route without changing this fail-safe contract.
- **Scope boundary:** Engineering may change only the demo start action, focused demo assertions, any minimal truthful guided-route copy required by this addendum, and living context in addition to the already-frozen family A16 repair. No credential readout/edit, Auth Admin mutation, password form, environment edit, database/SQL, Production action, owner prompt, roadmap change, or readiness increase is authorized.
- **Immediate handoff:** current distinct Engineering `/root/pm_combined_demo_a16_return/eng_demo_family_a16_return` owns both bounded demo regressions in one clean descendant. Fresh UX re-review and Independent QA must assess both; success in one cannot average away failure in the other.

### PM addendum - exact reviewed family handoff snapshot - 2026-08-13 00:24 -07:00

- **Role instance and returned evidence:** distinct Product Manager `/root/pm_combined_demo_a16_return` received the second Hosted Preview QA return for exact repair head `25ed0c9e01da3724b241dc9899724ea571ae298f`, tree `ddcbd8a59b1bab32b15cbfaa12dc6a7267bca2ab`, and non-production Preview `dpl_AaCm`. The previous local A16 and operator repair evidence remains preserved. Hosted QA proved the browser Receiver and Review stages can correctly show `Main Street Memorial Home`, one shared category `Identity details`, and a three-day window, then reproduced a blocking contradiction: `Create preview pass` opens the default Northstar/Elena pass with three shared categories and two private categories.
- **Root cause boundary:** activation writes only legacy `recipientId: 'saved-provider'` plus scope/expiry identifiers. `ActivePass` rejects every dynamic provider because it recognizes only the old static `RECIPIENTS`, then silently substitutes `FALLBACK_PASS` and `RECIPIENTS[0]`. The pass therefore renders stale defaults instead of the exact reviewed selection. The fallback is not recovery; it is a false success state.
- **Issue classification:** **FIX NOW**. This blocks the deterministic family demo and invalidates the exact Hosted Preview family completion cell. **WATCH:** no repair may weaken authenticated A16, reuse an initial/default provider, fabricate a durable receiver, or lose the existing storage-failure rollback. **BACKLOG:** none. **OWNER GATE:** none.

#### Frozen repair contract

1. On activation, construct one versioned, immutable browser-demo handoff snapshot from the **current reviewed state in that click**: current family intent when selected, exact current provider selection summary, exact selected scope IDs, exact expiry ID, activation instant, and server-independent derived expiry instant. Do not reread an initial prop, `DEFAULT_DRAFT`, `FALLBACK_PASS`, static `RECIPIENTS`, stale closure, or previously saved provider.
2. The receiver snapshot contains only the reviewed human fields needed by the pass: stable demo-local selection identity/source, display name, structured/formatted address, handoff availability truth, and the existing `Not contacted by Passage` boundary. It contains no Auth identity, raw token, family-space identifier, server receipt, durable-history claim, or authority grant.
3. The same snapshot object is serialized once to `passage.family.transfer.v1` and supplies the exact scope/expiry values sent to the browser sandbox command. The visible pass must derive receiver, categories, and deadline from that normalized snapshot only. There is no silent default substitution after a successful activation.
4. `ActivePass` validates fail closed: recognized snapshot schema/version, canonical timestamps and derived expiry, allowed intent, bounded provider fields, allowed unique scope IDs, and a recognized expiry. Invalid, missing, truncated, stale-version, or inconsistent storage renders an honest `This example handoff is unavailable` recovery with `Create it again`; it must not render Northstar, Elena, default categories, an active code, or a success/active claim.
5. Preserve the existing atomic recovery order. If session snapshot persistence fails, do not dispatch the sandbox command or navigate. If sandbox persistence fails after snapshot write, remove that exact snapshot and keep the reviewed selections in the composer. If cleanup fails, retain the explicit incomplete-copy/reset warning. No partial success is presented.
6. Preserve the current close transaction: the exact saved snapshot is removed before revocation dispatch and restored byte-for-byte when sandbox persistence fails. Successful close stays closed after reload and never recreates defaults.
7. Authenticated `/family` A16 search, selection preload, durable confirmation, RLS/authority, append-only receipt, replay/conflict, and failure behavior remain unchanged. This repair applies only to the browser-demo handoff snapshot/pass projection.

#### Acceptance and permanent regression matrix

1. **Discovered receiver:** choose Main Street Memorial Home, share only Identity details, choose three days, review those exact values, create the pass, and prove the pass shows Main Street Memorial Home and its reviewed New York address, exactly one visible category, exactly four private categories, and the same three-day/derived closing time before and after reload.
2. **Manual receiver:** enter a distinct manual name/address, select a different valid category set and access window, review, create, reload, and prove every receiver/category/deadline value is unchanged. Editing or changing the provider before activation uses the final reviewed provider only.
3. **Current intent:** selecting or changing Planning ahead versus Help needed today before activation stores the final current intent in the snapshot. If the pass does not render intent, the value remains normalized evidence only and must not alter receiver/scope/expiry projection.
4. **No default leakage:** source and browser tests fail if a successful saved pass uses `FALLBACK_PASS`, `RECIPIENTS[0]`, Northstar, Elena, three default scopes, or any initial-intent/default branch. Static fallback content is removed from the successful projection path.
5. **Atomic failure:** force session `setItem` failure and prove zero sandbox dispatch/navigation; force sandbox persistence failure and prove snapshot removal plus unchanged reviewed UI; force cleanup failure and prove explicit reset recovery. Existing activation and close atomic tests remain green.
6. **Corrupt/missing snapshot:** malformed JSON, unknown schema, changed expiry, unknown/duplicate scope, missing provider field, overlong provider field, and absent snapshot all render the unavailable recovery with no code, active state, protected link, or default receiver/content.
7. **Browser proof:** use clean contexts and capture Review immediately before activation, pass immediately after navigation, pass after reload, and closed state after reload at 1440, 390, and 360. Record exact final URLs, storage snapshot shape with private values redacted, network requests, console/page errors, unhandled rejections, failed requests, focus, target size, overflow, and 200 percent/320 reflow.
8. **Combined regression:** rerun provider zero-network browser-demo coverage, authenticated A16 source/data coverage, operator fallback coverage, public conversion, language, parity, Server Actions, runtime/deploy gates, TypeScript, optimized build, and `git diff --check`. No readiness credit follows from source repair alone.

#### Components, non-goals, and handoff

- Expected bounded files: `components/family/types.ts`, `TransferComposer.tsx`, `ActivePass.tsx`, the pure demo snapshot normalizer in `lib/presentation`, focused public/family tests, and this context. Touch `FamilyIntentJourney.tsx` only to pass the current selected intent into activation; do not redesign its UI. Do not touch A16 routes/actions, schema, migrations, RLS, RPCs, parity ledger, roadmap, environment, credentials, database, Vercel configuration, or Production.
- **Roadmap material-change classification:** NO. This restores exact state continuity in an already-required deterministic demo and changes no product direction, scope, milestone order, persona coverage, architecture doctrine, or score.
- **Immediate role:** prior distinct Development Engineer `/root/pm_combined_demo_a16_return/eng_demo_family_a16_return` resumes in a new role turn from exact `25ed0c9...`, implements only this addendum, runs the full gates plus local browser proof, appends the Engineering return, and freezes a Bot-authored `[skip deploy]` descendant for fresh independent QA. No owner prompt, database action, deployment, or Production action is required.

### PM addendum - guided operator plain language and skip target - 2026-08-13 00:29 -07:00

- **Confirmed defects and classification:** the same Hosted Preview return confirmed two additional **FIX NOW** defects on guided Director. Persona content exposes prohibited implementation narration including `after it is saved to the server`, `REAL WORKSPACE`, and `Authorized Northstar team after server save`. The focused skip link is approximately 39 CSS pixels tall. Staff and Vendor use the same internal narration pattern and shared shell, so all three guided personas require one bounded audit and repair. These defects are independent of credential drift and join the exact `25ed0c9...` descendant repair.
- **Plain-language contract:** guided Director, Staff, and Vendor pages describe only what the person sees and does in this private browser example, what changes on the current page, what resets on refresh, what was not sent/charged/changed, and who would own the next human step inside the example. Remove `server`, `server load`, `real workspace`, `authorized`, `durable`, `shared`, database/auth/deploy language, and any promise that another persona can already see page-local state. Do not replace it with softer infrastructure narration.
- **Facts and receipts:** replace the Director `Real workspace` fact with a human demo boundary such as `Example access / Elena only on this page`. Scope panels should say, for example, `This private example shows one director view. Try each step; changes stay on this page and reset on refresh.` Staff and Vendor receive persona-equivalent language. Action receipts state only the visible result on this page plus applicable no-send/no-purchase/no-payment truth; the existing page-only/reset sentence remains controlling.
- **Target-size contract:** the operations skip link must have a rendered minimum width and height of 48 CSS pixels in its focused visible state, with centered content, visible outline, no clipping, and no obstruction at 1440, 390, 360, and 200 percent/320 reflow. Apply the shared-shell fix without shrinking or shifting verified operator controls.
- **Permanent acceptance:** source tests fail on prohibited guided-persona phrases and require page-local/reset/no-send/no-payment boundaries for all three stories. Browser QA captures the complete visible text for Director, Staff, and Vendor, runs every action and reset, proves no cross-persona/shared claim, contained `/demo` navigation, 48-by-48 or larger skip link, keyboard focus order/visibility, zero overflow, and clean console/page/runtime evidence at required viewports. Verified server-authorized operator routes remain unchanged.
- **Bounded files:** Engineering may additionally change `app/demo/operator/OperatorDemo.tsx`, `components/operations/OperationsShell.module.css`, and focused public/operator tests. No other shell IA, authenticated operator copy, source authority, environment, database, deployment, roadmap, Production, or owner gate is in scope.

### Family handoff snapshot and operator language - Engineering return - 2026-08-13 00:43 -07:00

- **Role and handoff:** distinct Development Engineer `/root/pm_combined_demo_a16_return/eng_demo_family_a16_return` implemented the two frozen PM addenda on PM-only head `9a815b0189e68580554f5632a2ee8cd9fc4ecfcc`, a clean descendant of returned Engineering head `25ed0c9e01da3724b241dc9899724ea571ae298f`. No schema, migration, RLS, RPC, parity ledger, roadmap, environment, credential, database, Vercel, deploy, or Production resource changed.
- **Family repair:** browser-demo activation now serializes one immutable versioned reviewed snapshot containing the current intent, exact receiver and address, exact unique category IDs, access window, activation instant, and derived expiry. The demo pass normalizes that snapshot fail closed and shows an unavailable recovery for missing, corrupt, unknown-version, inconsistent-expiry, or invalid receiver/category content. Successful projection no longer substitutes Northstar, Elena, or legacy default scopes. Atomic activation cleanup and byte-for-byte close restoration remain intact.
- **Authenticated preservation:** route composition is explicit. `/demo/family/pass` selects `browser_demo`; `/family/pass` selects `authenticated`. Browser-demo activation alone creates the reviewed snapshot. Authenticated activation retains its prior `TransferDraft` serialization, scope/expiry dispatch values, legacy normalization, and example projection. A permanent source guard binds both route adapters.
- **Operator repair:** guided Director, Staff, and Vendor copy now describes only page-local actions, refresh reset, and applicable no-send/no-purchase/no-payment truth. Internal server, workspace, authorization, durable, and cross-persona narration was removed. The shared focused skip target now has centered content and a 48 by 48 CSS-pixel minimum.
- **Files changed:** `app/demo/family/pass/page.tsx`, `app/family/pass/page.tsx`, `app/demo/operator/OperatorDemo.tsx`, `components/family/ActivePass.tsx`, `FamilyIntentJourney.tsx`, `TransferComposer.tsx`, `types.ts`, `components/operations/OperationsShell.module.css`, `lib/presentation/demo-handoff-snapshot.js`, its declaration, `scripts/test-demo-handoff-snapshot.mjs`, `scripts/test-public-conversion.js`, `package.json`, and this context. Four pre-existing untracked browser artifacts remain untouched and excluded.
- **Engineering verification:** reviewed-snapshot/corruption matrix PASS; provider zero-network source guard PASS; public/demo boundary guard PASS; operator configured-sign-in fallback PASS; participant case, messaging, participant invitation source, parity, Server Action, persona-language, context, governance, operational-route, runtime-isolation, and deploy-decision guards PASS; direct TypeScript PASS; and `git diff --check` PASS before the final context append. Optimized Next.js 16.1.6 build PASS with 29/29 static pages and 51 routes immediately before the final explicit pass-route adapter split; focused family/operator guards and TypeScript passed after that split. The package wrapper itself was NOT RUN because its noninteractive dependency-purge prompt aborted before build; the installed Next binary completed successfully. The credential-bound P2 race runner was NOT RUN because `PASSAGE_P2_RACE_PROJECT_REF` is absent and failed closed before any connection.
- **Browser and release evidence:** local browser QA is NOT RUN per watchdog recovery. Fresh Independent QA owns discovered/manual receiver, review/pass/reload/close/corruption/atomic-failure, network capture, operator action/copy/skip-target, responsive, focus, overflow, and runtime cells. Hosted Preview QA is NOT RUN on this descendant. Production Deployment: NOT DEPLOYED. Production QA: NOT RUN. Overall release state: SOURCE REPAIR RETURNED FOR DISTINCT QA / NON-PRODUCTION / NO READINESS CREDIT.
- **Roadmap classification and auto-advance:** NO material change to direction, scope, milestone order, persona coverage, architecture doctrine, readiness doctrine, or score; no roadmap edit is required. Freeze one Passage Release Bot `[skip deploy]` commit and hand its exact head/tree to fresh Independent QA. No owner prompt is required.

### PM addendum - family pass hero receiver binding - 2026-08-13 00:55 -07:00

- **Role and returned evidence:** distinct Product Manager `/root/pm_combined_demo_a16_return` received the tiny Hosted/local QA copy return on exact source `5942d3fb58b2174a4424065465ee6a879320d69c`, tree `fef0cc1a3e7c0c12637ee4125202e3886c4ac5b3`. Receiver, category, and deadline continuity now pass, but the `/demo/family/pass` hero still renders the sandbox default sentence `Your handoff is ready for Northstar` after the validated snapshot receiver is Main Street Memorial Home.
- **Classification:** **FIX NOW**. This is one stale receiver label in an otherwise-correct browser-demo projection. It is not permission to change snapshot shape, activation, authenticated A16, sandbox authority, navigation, layout, or other copy.
- **Exact repair:** in `ActivePass`, browser-demo hero status copy must derive the receiver name from the already-normalized snapshot-backed `recipient.organization`. For an issued pass it says `Your handoff is ready for {validated receiver}.` Any accepted/in-progress/proof-submitted browser-demo hero status must also use truthful browser-example language and the same validated receiver when naming a receiver; it must never call `selectFamilyStatus` for a receiver label that can leak Northstar. The authenticated `/family/pass` continues to use its existing authenticated/sandbox receiver source and `selectFamilyStatus` behavior unchanged.
- **Fail-closed boundary:** browser-demo hero copy is rendered only after the existing valid snapshot, recipient, expiry, and closing-time gate. Missing or corrupt snapshot continues to render `This example handoff is unavailable`, with no receiver name, Northstar fallback, hero, code, or active claim.
- **Permanent acceptance:** focused source tests prove browser-demo hero binds `recipient.organization`, forbid `familyStatus.update` as its browser-demo branch, and preserve authenticated `familyStatus.update`. Browser QA creates or loads a valid Main Street snapshot, proves the hero and prepared-for area both say Main Street and contain no Northstar, then repeats after reload. A missing/corrupt snapshot proves the unavailable recovery contains no fabricated receiver. Existing manual/discovered continuity, atomic failure, provider, operator, TypeScript/build, and combined gates rerun proportionally.
- **Scope and handoff:** expected product change is only `components/family/ActivePass.tsx`, its focused regression assertion, and this append-only context. No Production, database, environment, deployment, roadmap, owner gate, or readiness increase. Prior Engineer resumes on exact `5942d3f...`, freezes one Bot-authored `[skip deploy]` descendant, and returns it to fresh Independent QA immediately.

### Family pass hero receiver binding - Engineering return - 2026-08-13 01:02 -07:00

- **Role and exact input:** distinct Development Engineer `/root/pm_combined_demo_a16_return/eng_demo_family_a16_return` received the tiny PM FIX NOW addendum through PM-only head `511736047eff00392f747ce87d01cdc83275433f`, a clean descendant of source repair `5942d3fb58b2174a4424065465ee6a879320d69c`.
- **Repair and preservation:** the browser-demo hero now derives issued, accepted, and proof-submitted status copy from the already-validated `recipient.organization`; it cannot consume the sandbox default Northstar sentence. The fail-closed missing/corrupt snapshot return remains before hero, receiver, and code rendering. Authenticated mode explicitly retains `familyStatus.update` unchanged.
- **Files and verification:** changed only `components/family/ActivePass.tsx`, `scripts/test-public-conversion.js`, and this context. Snapshot corruption guard PASS; public/demo boundary and new mode-bounded hero assertions PASS; provider zero-network source guard PASS; operator fallback PASS; frontend/backend parity PASS; Server Action guard PASS; persona-language PASS; runtime-isolation PASS; deploy-decision PASS; direct TypeScript PASS; and `git diff --check` PASS. Optimized build and browser QA were NOT RUN for this tiny descendant per watchdog commit direction; fresh Independent QA owns Main Street hero/prepared-for/reload and corrupt-snapshot browser cells.
- **Release state:** Source QA: ENGINEERING FOCUSED GATES PASS / DISTINCT QA NOT RUN. Hosted Preview QA: NOT RUN on this descendant. Production Deployment: NOT DEPLOYED. Production QA: NOT RUN. Overall release state: SOURCE-ONLY REPAIR RETURNED FOR DISTINCT QA / NO READINESS CREDIT.
- **Roadmap and next role:** NO material direction, scope, milestone-order, persona-coverage, architecture, readiness-doctrine, or score change; roadmap unchanged. Freeze one Bot-authored `[skip deploy]` commit and immediately hand exact head/tree to fresh Independent QA. No owner prompt is required.

### PM Sprint Brief - final operational consolidation candidate - 2026-08-13 01:28 -07:00

- **Role instance and controlling evidence:** distinct Product Manager `/root/pm_combined_demo_a16_return` received bounded Hosted Preview QA on exact demo-repair source `2eac2bca021a9c7e25223874d96578c7701c68ec`, tree `b72e1694ca75205baf0ba6e140962833a6bcbdfd`, Preview `dpl_BiY9`. The 390-pixel Main Street activation/reload hero, corrupt/missing snapshot recovery, guided operator action, reload reset, console, and page-error cells passed. Overall Hosted Preview remains **PARTIAL** because Vercel runtime logs were not retrieved and authenticated-session provenance for `/family/pass` was not independently proven. Production was not deployed or tested.
- **Responsive packet:** exact independently source-QA-passed responsive polish is represented by integration merge `7cf432cdbc3cafd07d0b3242f0c3480b9cd3db30`, tree `3505386156f4b8f935facd812a5c496002e90c16`, whose second-parent lineage ends at `b26d619100da482995c0e2a41ae46846498a2451`. It contains the bounded Director/Staff hierarchy changes, responsive hierarchy regression, and the corrected urgent migration parity reference. Its merge base with `2eac2bc...` is exact combined candidate `0065bd41c8191a0c8bed1dc60528d76c2d6ac5a5`.
- **Issue classification:** **FIX NOW:** consolidate these two exact descendants without losing any combined authority or demo repair, then run one fresh complete Source/Data/Hosted candidate. **QA INFRASTRUCTURE DEBT:** real authenticated family/participant/messaging provenance and Vercel runtime-log evidence remain open. **DEPLOY CONFIG DEBT:** the hidden Director credential has drifted; guided local fallback prevents a demo dead end but does not prove real Director authentication. **CURRENT IDENTITY TRUTH:** Avery is the active Staff persona; the historical Cycle 7A Staff identity is revoked and must be used only as a denial cell. **OWNER GATE:** none.

#### Sprint goal

Freeze one exact non-production Passage Zero candidate that contains the full combined P2 participant lifecycle, least-privilege case authority from PR78, workflow messaging, A16 provider discovery, public/conversion and browser-demo repairs, operator safe fallback, exact family handoff continuity, responsive Director/Staff hierarchy, and urgent parity correction, then prove it through fresh complete source, isolated data, exact Preview, real-sign-in, runtime, responsive, recovery, and denial evidence.

#### Integration contract

1. Start from exact `2eac2bc...`. Merge exact `7cf432c...` as a recorded two-parent consolidation. Do not manually reimplement its responsive files or cherry-pick an incomplete subset unless Git proves the merge impossible. Preserve both append-only context histories and both `package.json` test commands when resolving the only expected documentation/package collisions.
2. After merge, verify all `2eac` product/test blobs remain unchanged except mechanically necessary conflict resolution. In particular preserve `app/demo/actions.ts`; browser family route/pass composition; `ActivePass`; family intent/composer/provider discovery; versioned demo snapshot normalizer; operator plain-language/fallback; 48-pixel operator skip; provider types; and demo/provider/operator/public tests.
3. Verify all `7cf` product/test blobs are present exactly: Director page hierarchy, Director team identity/account-state hierarchy, Staff hierarchy, shared responsive operations CSS, `test-responsive-hierarchy`, and corrected frontend/backend urgent migration reference from `b26d619...`.
4. Preserve the exact combined base capabilities and data lineage already present under `0065`: PR78 least-privilege case authority; P2 participant invite/rotate/decline/cancel/revoke/reload lifecycle; messaging owner/participant/Director/assigned-Staff behavior and denials; A16 authenticated durable provider discovery plus browser-demo-local adapter; public/conversion routes; guided Director/Staff/Vendor fallback; all 36 unique migrations and existing parity rows.
5. Do not change schema, migration bodies, RLS, RPCs, fixtures, provider data, runtime variables, credentials, Vercel project settings, Production, pricing, roadmap scores, or persona scope during integration. Any unexpected product conflict returns to PM instead of being guessed through.

#### Candidate identity and source acceptance

- One Bot-authored branch `integrate/final-operational-candidate-20260813`; one exact `[skip deploy]` source head/tree; no direct `main` push and no Production deploy marker.
- `git merge-base`, parent list, tree diff, migration inventory, and targeted blob hashes prove both descendants and the exact `0065` base are incorporated with no missing or duplicate migration.
- Fresh candidate gates: public conversion; browser provider discovery; reviewed handoff snapshot/corruption/atomic recovery; operator configured-sign-in fallback; responsive hierarchy; participant P1/P2 source and race guards; case authority; workflow messaging; parity fixture/integration and full ledger; Server Action exports; persona language; agent context; release governance; operational routes; runtime isolation; Vercel deploy decision; React/Next review; direct TypeScript; optimized Next build; and `git diff --check`.
- Roadmap material-change classification: **NO**. This consolidates previously scoped, independently reviewed work and closes its evidence loop. It changes no direction, milestone order, persona coverage, architecture doctrine, readiness doctrine, pricing, or score. The roadmap must not be edited merely to manufacture freshness.

#### Fresh isolated Source/Data QA

Independent QA must bind every run to candidate SHA/tree and isolated Supabase `uyacxqtsiwlvtmhxvoxr`; Production `qsveqfchwylsbncsfgxe` is forbidden.

1. Apply or verify the exact 36-migration lineage through migration tooling only; reject duplicate, missing, reordered, or changed bodies. Record exact migration names/hashes and cleanup behavior.
2. Run PR78 case authority, messaging rollback/authority/replay/conflict, A16 authority/concurrency/reversibility, participant P1/P2 lifecycle/reversibility, five ordinary-session races, advisors, exact verifier, and zero-residue cleanup. Record durable row/event cardinality and append-only update/delete denial.
3. Personas and denials: Maya owner; Dana active updates participant; revoked participant; wrong-case/wrong-user; exact active Director; Avery as active assigned Staff; historical revoked Cycle 7A Staff; wrong organization/location/unassigned Staff; Vendor privacy. Do not substitute the revoked historical Staff for Avery in success cells.
4. Data QA must prove authenticated family provider selection, participant access, messaging state, and operator assignment come from the isolated database and exact identities, not browser sandbox storage or a warmed tab.

#### Exact Preview and runtime provenance

- After complete Source/Data QA and the required role chain approve the exact head, Deploy may publish one branch-only non-production Preview from the canonical Vercel project `prj_b7CKwanQaKwFQSHInr3l6wsZy9nD`, bound only to isolated project `uyacxqtsiwlvtmhxvoxr`. Production aliases, values, and project remain untouched.
- Record candidate SHA/tree, Git branch, Vercel deployment ID, unique Preview URL, project/team, build result, environment/project-ref binding, deployment creation/ready timestamps, and final route provenance before and after QA.
- Runtime logs must be retrieved through the Vercel connector or authenticated Vercel dashboard for the exact deployment and QA time window. Local CLI authentication failure is not runtime evidence and must not be retried as the controlling path. Capture warnings/errors/fatals plus relevant request traces without exposing secrets or persona credentials.
- Hidden Director credential drift remains Deploy-owned configuration debt. Safe recovery paths are official Auth Admin on the isolated project and branch-scoped sensitive Preview configuration without reading or exposing the secret. Browser password forms, owner credentials, Production Auth, copied hidden values, or ad hoc database writes are prohibited. If the network boundary blocks recovery again, preserve the failure and continue all other candidate cells; the real Director success cell remains PARTIAL, while guided Director fallback is tested separately and may pass.

#### Full Hosted real-sign-in matrix

Use fresh, independent browser storage contexts and record exact identity provenance with sensitive values redacted.

1. **Public/conversion:** home, pricing, guides/blog, Story/mission, trust, resources redirects, care providers, funeral-home paths; direct and applicable client navigation; purpose/action/comprehension; no hydration/runtime/request errors.
2. **Browser demo:** family planning/urgent entry; discovered Main Street and manual receiver; final current intent; exact scopes and deadline through Review -> pass -> reload -> close -> reload; corrupt/missing/storage-failure recovery; zero authenticated provider endpoint calls. Guided Director, Staff, Vendor actions/reset/fallback, plain language, contained navigation, no shared/durable claims.
3. **Authenticated family/A16:** real Maya sign-in; selection preload; search; explicit confirmation; durable receipt/audience; reload/cross-session truth; manual/replacement; provider failure/retry; direct route and sign-out boundary. Bind exact DB rows/events.
4. **Participant lifecycle/access:** real Dana invite inspection/acceptance or current active access as applicable, bounded Updates projection, reload, rotation/decline/pending cancel/accepted revoke, expiry, replay, wrong-user and wrong-case denial, revoked access removal. Bind exact invitation/grant/event cardinality.
5. **Messaging/case:** Maya owner and Dana active participant see only their authorized case/message projection; Director and Avery assigned Staff see their role-bounded task/message behavior; wrong user, wrong case, unassigned/revoked Staff, revoked participant, replay/conflict and reload paths fail closed. Bind exact message/event rows and timestamps.
6. **Director/Staff responsive operations:** real Director if credential safely recovered; Avery active Staff success; historical Cycle 7A Staff denial; hierarchy, team identity once, account state, primary/supporting facts, assignment/start/proof/review/reload and direct-route authority. Guided fallback remains a separate browser-only cell and never substitutes for real authorization evidence.
7. **Vendor/operator:** guided Vendor privacy and actions; any real authorized Vendor path already supported by the candidate; no family-record browsing; no payment/send claims; denial/recovery and reload truth.
8. Every applicable page and journey runs at 1440, 390, and 360, plus 200 percent text and 320-pixel reflow for critical forms/pass/operator work. Verify at least 48-by-48 enabled targets, overflow, wrapping, keyboard order, focus visibility/return, announcements/alerts, landmarks/headings/names, reduced motion, console warnings/errors, hydration, page errors, unhandled rejections, failed requests, final URL, and runtime logs.

#### Verdict rules, risks, and non-goals

- A Vercel READY build, HTTP 200, source PASS, demo fallback, or one responsive viewport cannot substitute for real authenticated provenance, isolated data cardinality, logs, or the complete matrix. Missing Director credentials keep only that real-auth cell and the whole checkpoint evidence PARTIAL; they do not invalidate truthful guided fallback or other passed cells.
- Any contradiction invalidates the affected earlier verdict and returns the exact candidate to PM. No averaging across personas and no whole-platform checkpoint increase until all domain floors and required cells pass.
- Non-goals: no Production deployment/QA; no `main` merge; no schema or migration edit; no paid/live provider; no real external email/SMS; no pricing/billing; no destructive data; no material legal/privacy/security decision; no owner prompt; no new UI feature beyond exact consolidation and evidence recovery.

#### Immediate role chain

1. Distinct Integration Engineering merges exact `7cf432c...` into exact `2eac2bc...`, resolves only bounded collisions, runs complete candidate source gates, records hashes/files/verdicts, and freezes one Bot-authored `[skip deploy]` head.
2. Fresh independent Source/Data QA reruns the complete exact-candidate source, isolated SQL/RLS/race/reversibility/advisor matrix and cleanup.
3. Independent Agent Review and Development Head bind the exact QA-passed head. Deploy creates one isolated Preview; fresh Hosted QA runs the full matrix above; runtime evidence comes from the Vercel connector/dashboard.
4. PM receives PASS/PARTIAL/FAIL and scopes the next gap immediately. No Production action and no owner gate are authorized by this brief.
- Bug (found in docs/evidence/passage-zero/qa-2026-08-10-full-sweep.md, P0, tested against the release/10h-delivery superset preview): every active continuity_participants family member who isn't the continuity-space owner was locked out of /case/[id]/today (and, on branches carrying the not-yet-merged messaging feature, /case/[id]/messages too -- this branch, greenfield/passage-zero, has no messaging feature yet, so only the case-detail half applies here). Root cause: migration participant_updates_case_scope (2026-07-30, applied to the shared isolated project this branch's preview also reads from) correctly narrowed passage_private.can_view_workflow_as_family() to owner-only and shipped public.list_participant_family_updates() as the participant-safe replacement, but that function has no workflow_id in its input or output, and lib/family/case-view.ts still gated on the now-owner-only raw `workflows` table read.
- Fix (branch `fix/participant-case-access-greenfield`, off `greenfield/passage-zero`; same fix also landed separately as `fix/participant-case-access` off `release/10h-delivery`, PR #77, for the messaging half):
  - `supabase/migrations/20260810230000_participant_case_update_for_workflow.sql` -- adds public.get_family_case_update_for_workflow(p_workflow_id uuid), an additive, workflow-id-scoped sibling to list_participant_family_updates(). Does not modify any existing function or RLS policy.
  - `supabase/migrations/20260810230100_participant_case_update_for_workflow_grant_hardening.sql` -- revokes the implicit PUBLIC/anon EXECUTE grant CREATE FUNCTION adds by default.
...
## P1 signed-in hydration repair - Development Engineering handoff - 2026-08-13 09:50 -07:00

- **Role and handoff:** distinct Development Engineer `/root/pm_final_hydration_return` received the frozen PM Sprint Brief and UI/UX reliability bar for exact candidate `969244ddad6041afe545d415207342da2801bf1b`, tree `fe4708978fd3b4618a143472b44ef89ddf39cb61`, and invalidated Preview `dpl_5AcmTJms8ajoSBvXUUs3SGvsyhoY`. Roadmap material-change classification is **NO**. No roadmap change was made.
- **Exact diagnosis before edit:** fresh clean-profile Hosted QA reset its page-error accumulator before each navigation. Owner `/family` emitted React `#418`; raw versus hydrated structure first differed where the client inserted `small.intentBoundary` only after the `FamilyIntentJourney` mount effect set `restored=true`. Owner `/case/c7b10001...0001/today` emitted fresh `#418` at 1440 and 390; its raw authenticated response contained the route-level `YOUR CASE / Opening this case...` loading `main`, while the browser's first resolved tree began with the case navigation and bounded Rivera data. Owner Messages also reproduced at 1440/360 during the invalidated-candidate run; active participant Today and observed director/staff/vendor controls were clean. Exact React error URL was `https://react.dev/errors/418?args[]=text&args[]=`. This is deterministic render-boundary evidence, not wrong case data or authority.
- **Smallest implementation:** `components/family/FamilyIntentJourney.tsx` now renders the existing browser-demo disclosure in the initial server and client tree instead of inserting it after mount. The obsolete `restored` state and effect write were removed; visible copy, intent restoration, storage recovery, actions, and layout are unchanged. `app/case/[id]/today/loading.tsx` was removed so the authenticated dynamic Today route does not stream a competing page-level `main` before its resolved authorized tree. No owner, participant, message, data, RLS, Auth, schema, fixture, environment, deployment, or Production behavior was broadened.
- **Permanent regression:** new `scripts/test-hydration-determinism.mjs`, bound as `pnpm test:hydration-determinism`, requires the demo boundary in the unconditional initial tree, rejects the prior effect-gated form, and rejects reintroduction of the conflicting Today route-level fallback.
- **Engineering verification:** hydration determinism, TypeScript, participant case-scope 25 assertions, messaging security 27/27, persona language, parity 22/22 plus Server Action exports, runtime isolation, operational route gate, and diff checks pass. Default Turbopack build hit only the already owned Windows generated-path-length panic in the long nested worktree. The official `next build --webpack` recovery compiled successfully, ran TypeScript, generated 29/29 static pages, collected traces, and emitted the complete 51-route manifest.
- **Local browser smoke:** a clean installed-Edge agent-browser session opened the optimized local `/family`; the existing disclosure rendered exactly once and the page produced no captured page or console error. Local provider data was unavailable under the intentionally unconfigured local runtime and is outside this hydration repair. An authenticated local owner case session was not established; exact owner Today/Messages, participant controls, responsive cells, and runtime logs remain mandatory fresh Hosted QA on a new immutable non-production Preview.
- **Recovery and boundaries:** source recovery is one revert of this bounded commit. No data recovery applies. No Supabase, credential, broker, Vercel, alias, environment, external communication, Production database, or Production deployment change occurred. No masking strategy, `suppressHydrationWarning`, disabled SSR, client-only shell, hidden duplicate content, copy reduction, or instrumentation change was used.
- **Next roles:** fresh distinct Source QA must bind the exact head/tree, challenge the diagnosis and four-file diff, rerun the focused/full gates and official build, and confirm authority/privacy preservation. Only after Source QA may non-production Deploy create a new exact-head Preview. Fresh Hosted QA must use independent clean owner/participant contexts, reset errors per navigation, and run the complete PM/UX 1440/390/360/native-200 direct/client/reload matrix. The invalidated old Preview remains historical evidence and may not regain approval.

Release truth: **Source QA: ENGINEERING GATES PASS / DISTINCT QA NOT RUN. Hosted Preview QA: INVALIDATED / FAIL FOR OLD `dpl_5Acm`; NOT RUN FOR REPAIR HEAD. Production Deployment: NOT AUTHORIZED / NOT CHANGED. Production QA: NOT RUN. Overall release state: SOURCE ONLY REPAIR / OLD PREVIEW INVALIDATED / FRESH QA REQUIRED.**

### QA return and Messages hydration completion - Development Engineering - 2026-08-13

- Fresh Independent Source QA `/root/qa_hydration_347` returned exact repair head `3474126bdd01f26b104bc77563b6b973d4b70d22`, tree `d994d3becf907ad716a9789af568bdbfd96c9f39`, as **FAIL** even though every source gate, TypeScript, and official webpack build passed. The correct release-blocking finding was that owner Messages had independently reproduced React `#418`, while the first repair left Messages byte-identical and its guard covered only `/family` and Today. No Preview was authorized.
- Engineering preserved the valid `/family` and Today corrections and completed the shared-cause diagnosis. `app/layout.tsx` globally wrapped every route in the browser-demo `PassageZeroProvider`. Its mount effect reads localStorage and replaces provider state above the active route. Owner QA had previously used `/family` and carried browser-demo state; the clean participant profile did not. Authenticated Today and Messages never call `usePassageZero`, so this global post-mount state transition was unnecessary and could race their streamed RSC hydration. Messages itself has no client clock, locale formatter, storage/viewport branch, or client-generated visible text; its message time labels are explicit server UTC strings.
- Smallest correction: RootLayout now renders children directly. `PassageZeroProvider` is scoped only around the seven entry surfaces that actually render a sandbox consumer: `/family`, `/family/pass`, `/demo` reset, `/demo/family`, `/demo/family/pass`, `/receive`, and `/director/intake`. Director intake was split into a provider-owning entry component and an inner hook consumer. Existing demo state, local persistence, reset, handoff, receive, and intake behavior remain unchanged. Authenticated case Today and Messages now sit outside browser-demo state entirely.
- The permanent hydration guard now proves: the root layout cannot reintroduce the sandbox provider; all seven real consumer entrypoints retain a scoped provider; authenticated Messages imports no sandbox provider/hook; MessageThread initial text derives from no time, randomness, locale, browser, or storage input; the `/family` disclosure stays in the initial tree; and the competing Today loading boundary stays absent.
- Verification PASS: hydration determinism; direct TypeScript; workflow messaging 27/27; participant case scope 25 assertions; public conversion and demo boundary/recovery; demo handoff snapshot/corruption recovery; configured operator fallback; persona language; parity 22/22 plus Server Action export guard; runtime isolation; operational route gate; and diff checks. Official Next.js 16.1.6 webpack build compiled, ran TypeScript, generated 29/29 static pages, collected traces, and emitted the complete 51-route manifest. Only the known nested-worktree lockfile-root warning remained.
- No database, schema, migration, RLS, RPC, fixture, Auth, credential, broker, environment, Vercel, alias, external communication, Production database, Production deployment, roadmap, copy, or layout change occurred. Recovery is a source-only revert of this descendant. The old Preview remains invalidated evidence.
- Next role: fresh distinct Source QA binds the exact descendant head/tree, challenges the provider-to-Messages diagnosis and complete consumer map, reruns the full gates/build, and checks the diff for preserved demo and authority behavior. Only a passing independent result may hand off to non-production Deploy and new exact-head Hosted QA.

Release truth: **Source QA: ENGINEERING GATES PASS / FRESH DISTINCT QA NOT RUN. Hosted Preview QA: INVALIDATED / FAIL FOR OLD `dpl_5Acm`; NOT RUN FOR THIS DESCENDANT. Production Deployment: NOT AUTHORIZED / NOT CHANGED. Production QA: NOT RUN. Overall release state: SOURCE-ONLY QA-RETURN REPAIR / FRESH QA REQUIRED.**

## P1 signed-in hydration repair - isolated Preview deployment - 2026-08-13 10:13 -07:00

- **Role and authority:** distinct Deploy Agent `/root/pm_final_hydration_return` received fresh independent Source QA PASS for exact Bot-authored repair head `1f1822a8b8a6350f7bb7df791fd28f1197b22902`, tree `ec1bb16756a2c3bf58e7d2847caf6ec703efa05b`. That verdict authorized exactly one isolated non-Production Preview. Production, environment mutation, and data mutation remained prohibited.
- **Published lineage:** the exact source branch `integrate/final-operational-candidate-20260813` was pushed without changing its approved head. Its `[skip deploy]` Git-integration attempt was correctly canceled. Deploy then created Bot-authored merge trigger `f536de7e349594fbee3e273a050ad218ef93c9c6` on `release/10h-delivery`, with parents `1857cc82bbda3029493611c40cf14d85395cf373` and `1f1822a8b8a6350f7bb7df791fd28f1197b22902`, message `chore: verify hydration determinism repair [deploy] [qa-approved]`, and exact application tree `ec1bb16756a2c3bf58e7d2847caf6ec703efa05b`. The trigger tree therefore byte-matches the Source-QA-approved repair tree.
- **Canonical Preview:** Vercel project `prj_b7CKwanQaKwFQSHInr3l6wsZy9nD` created immutable Preview deployment `dpl_6BpVXVNUbLB7zPgf7uEkehnKtHyA` at `https://thepassageappio-n2p9h6jen-thepassageappio-7018s-projects.vercel.app`. Vercel metadata binds it to Git SHA `f536de7e349594fbee3e273a050ad218ef93c9c6`, ref `release/10h-delivery`, Passage Release Bot authorship, canonical project, and Preview target (`target: null`). State reached `READY`; this is build/deploy evidence, not Hosted QA approval.
- **Build and isolation evidence:** Next.js `16.1.6` compiled successfully, TypeScript passed, `29/29` static pages generated, the complete 51-route manifest included owner Today and Messages, and deployment completed in 29 seconds. The errors-only build-log query contained no build error. A fresh protected fetch of immutable `/login` returned HTTP 200 and its server RSC payload bound `supabaseUrl` to isolated project `https://uyacxqtsiwlvtmhxvoxr.supabase.co`. No Production `qsveqfchwylsbncsfgxe` binding appeared or was accessed.
- **Runtime evidence and recovery:** the deployment-scoped runtime log recorded `GET /login 200` on `release/10h-delivery`; the same deployment's error/fatal query returned no entries. No alias promotion, Production deployment, environment edit, credential change, Supabase mutation, or external communication occurred. If Hosted QA fails, hold this immutable Preview, return the exact finding to Product Manager/Engineering, and repair or revert in source; Production requires no rollback because it was untouched.
- **Next distinct role:** Hosted Preview QA must use this immutable deployment and independent clean owner and participant contexts. It must reset page/console error capture per direct navigation, client navigation, and reload; verify owner `/family`, Today, and Messages plus participant controls at 1440, 390, 360, and native 200% where the shared component applies; record final URLs, rendered bounded purpose/action, overflow, hydration/page/console errors, unhandled rejections, failed requests, exact deployment/Git/tree/backend binding, browser, reviewer, and timestamps. React recovery or any missing evidence cell is FAIL or PARTIAL, never PASS by inference.

Release truth: **Source QA: PASS for `1f1822a8` / tree `ec1bb167`. Hosted Preview QA: NOT RUN for `dpl_6BpVXVNUbLB7zPgf7uEkehnKtHyA`. Production Deployment: NOT AUTHORIZED / NOT CHANGED. Production QA: NOT RUN. Overall release state: PREVIEW DEPLOYED / HOSTED QA REQUIRED / NON-PRODUCTION PARTIAL.**
### Final operational consolidation - Integration Engineering handoff - 2026-08-13 02:18 -07:00

- **Role and received handoff:** distinct Integration Development Engineer `/root/pm_combined_demo_a16_return/eng_final_operational_merge` received the frozen PM Sprint Brief at `e96bdb73a37184ab921a25e6568c62e899672559`. The exact demo-repair lineage is `2eac2bca021a9c7e25223874d96578c7701c68ec`; the exact responsive lineage is `7cf432cdbc3cafd07d0b3242f0c3480b9cd3db30`; their exact merge base is `0065bd41c8191a0c8bed1dc60528d76c2d6ac5a5`. Roadmap material-change classification is **NO**. No roadmap edit was made.
- **Recorded consolidation:** exact responsive head `7cf432c...` was merged with `--no-ff` into PM head `e96bdb7...`. The recorded Bot-authored merge is `4fcfb4d4d25d5cc3952f8e0955d77ff95c827e02`, tree `dd31d911375ea1fc2e22c6410eb47a4bf3d649f9`, parents `e96bdb73a37184ab921a25e6568c62e899672559` and `7cf432cdbc3cafd07d0b3242f0c3480b9cd3db30`. The only manual conflict was append-only union of this context. `package.json` auto-merged and preserved the demo, provider, operator, participant, messaging, parity, server-action, governance, runtime, deploy, and new responsive test commands.
- **Preserved product and data lineage:** targeted Git blob comparison found no mismatch between the consolidated tree and `2eac2bc...` for the frozen demo actions, both family route adapters, `ActivePass`, intent/composer/provider discovery, family/provider types, operator page-local fallback, shared operations shell, handoff normalizer, and focused demo/provider/operator tests. Targeted comparison found no mismatch with `7cf432c...` for Director, Team, Staff, shared responsive CSS, responsive regression, and the corrected urgent parity ledger. `2eac2bc...`, `7cf432c...`, and `0065bd41...` are all ancestors. The candidate retains exactly 36 uniquely named migration blobs, including urgent boundary `20260727042651` and A16 index hardening `20260813043536`; no migration, schema, RLS, RPC, fixture, provider record, parity body, environment, credential, or database changed during integration.
- **Source gates:** fresh candidate source gates passed: public conversion and browser-demo boundary; browser provider discovery; reviewed handoff snapshot and corruption/atomic recovery; configured operator sign-in fallback; Director/Staff responsive hierarchy; participant invitation P1/P2 `42/42`; participant case scope `25` assertions; workflow messaging `27/27`; parity `22/22`; use-server exports; persona language; agent context; release governance; operational route; runtime isolation; Vercel deploy decision; direct TypeScript; and `git diff --check`. React/Next best-practices review found no integration-introduced critical waterfall, bundle, server-action authorization, client listener, derived-state, nested-component, or accessibility regression; the merged server components remain server components and the focused source guards bind target size, headings, labels, recovery, and mode separation.
- **Cross-platform source-guard repair:** the first public-conversion run failed because its source reader retained Windows CRLF while two assertions used literal LF substrings. Product source visibly preserved the required authenticated-only branch. The focused test reader now normalizes CRLF to LF at the read boundary. The full source matrix then passed. This changes no product behavior or contract.
- **Build and QA-infrastructure evidence:** direct TypeScript passed. Default `next build` reached Turbopack and failed only because the generated `ActivePass` source-map path exceeded the Windows filesystem maximum under the deliberately long isolated worktree. QA infrastructure fix-it owner is Integration Engineering; affected cell is the default Turbopack optimized-build command; target is this candidate freeze; recovery test is the official exact-tree `next build --webpack` fallback or a genuinely short physical checkout. The official webpack recovery passed compilation in 47 seconds, TypeScript, page-data collection, `29/29` static pages, build traces, and the complete 51-route manifest. No source or Next configuration was changed to obtain the recovery.
- **Files changed by integration:** responsive lineage contributed `app/director/page.tsx`, `app/director/team/page.tsx`, `app/staff/page.tsx`, `app/operations-beta.module.css`, `scripts/test-responsive-hierarchy.js`, `package.json`, and its append-only context. Integration Engineering changed only `scripts/test-public-conversion.js` for newline-normalized reads and this evidence append. No product implementation was reimplemented or broadened.
- **Release truth and next role:** Source QA: **ENGINEERING GATES PASS / DISTINCT QA NOT RUN**. Hosted Preview QA: **NOT RUN** on the final consolidation head. Production Deployment: **NOT DEPLOYED**. Production QA: **NOT RUN**. Overall release state: **SOURCE ONLY / RETURNED FOR FRESH INDEPENDENT SOURCE AND ISOLATED DATA QA / NO READINESS CREDIT**. Fresh independent Source/Data QA must bind its work to the exact final head/tree, verify the exact 36-migration inventory against isolated Supabase `uyacxqtsiwlvtmhxvoxr`, run the complete PR78, messaging, A16, participant lifecycle/race/reversibility/advisor/cleanup matrix, and preserve Production `qsveqfchwylsbncsfgxe` untouched. No owner prompt, deploy, Preview, Production, environment, or credential action is authorized by this Engineering handoff.
