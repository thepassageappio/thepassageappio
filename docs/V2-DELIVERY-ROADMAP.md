...
## P2 legal and vendor-risk prep packets — September 7, 2026

Two counsel-handoff prep documents for the P2 production-hardening gate are complete and ready for Steve to hand to outside counsel and to a pilot institution's procurement team, respectively. Both were built by product/engineering directly from this repository's own product copy, source code, and database migrations — neither is the actual legal review, independent security assessment, or vendor-risk attestation the P2 gate still requires; both say so explicitly in their own text.

- **[LEGAL-REVIEW-BRIEFING-PACKET-2026-09-07.md](./LEGAL-REVIEW-BRIEFING-PACKET-2026-09-07.md):** what Passage Authority does and its established workflow-coordination-only / not-a-legal-validity-determination boundary, quoted verbatim from `src/app` product copy and `docs/PRODUCT-SOURCE-OF-TRUTH.md`; the open legal questions P2 requires counsel to resolve (state POA formalities, ESIGN Act treatment of the evidence-upload flow, notarization checks Passage doesn't perform); and a state-by-state POA formality gameplan for New York (the only current P1 target state) plus four reasonable near-term-expansion states (New Jersey, Connecticut, Massachusetts, Pennsylvania), each assessed against what the evidence-upload/review flow actually captures today. The state-law research is from general web sources, explicitly flagged as non-primary-source and requiring counsel verification — it is not legal advice.
- **[VENDOR-RISK-ASSURANCE-PACKET-2026-09-07.md](./VENDOR-RISK-ASSURANCE-PACKET-2026-09-07.md):** a bank/credit-union procurement-ready security assurance packet, built by auditing the real Supabase migrations and `src/` code for authentication, RLS, encryption, audit logging, and evidence-storage controls (each cited to a specific migration, policy, or table), a plain non-euphemistic gap list (no SOC 2, penetration test, encryption/key-management review, retention controls, or audit export yet — matching the product's own public security page's self-disclosed gaps rather than contradicting it), and a scoped privacy review of what personal/sensitive data is collected, where it's stored, and the current retention/deletion gap.

Neither packet changes the P2 exit-evidence requirement above: outside counsel review and an independent scoped security assessment have not occurred as of this date and must happen before Passage accepts real institution POA, participant, account, or authority data, or begins live-business onboarding.
