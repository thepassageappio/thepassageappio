...
- Release state, September 5: 131 domain and security tests, TypeScript, ESLint, optimized production build, 44 public routes, and eight participant-link recovery states pass. Vercel production deployment `dpl_BWHK6ZdbLB7UivCd3L3jppvNFyFY` is ready on `demo.thepassageapp.io` and adds visible feedback when browser validation blocks a required-field submission.
