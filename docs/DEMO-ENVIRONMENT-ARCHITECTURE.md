...
- Demo now includes namespaced `demo_run` state, an owner/admin-only service command, exact presenter and recipient allowlists, optimistic version checks, idempotent replay, and append-only run/audit evidence. Its local database replay passes without deleting prior evidence, sending messages, consuming an activation, changing membership, or crossing organization boundaries. The migration and presenter allowlist are deployed, and an authenticated owner prepared a fresh run with desktop/390px/360px verification. Still required: add a separately authenticated reviewer, complete the remaining negative-path matrix, and rehearse the seven-minute story.
