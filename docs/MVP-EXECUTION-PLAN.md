...
**Current score:** 9.6 of 10 to a fully functioning hosted synthetic MVP
...
| P0 | Repeatable enterprise demo | A fresh synthetic organization and request can be reset and completed from institution signup through revocation in 7 minutes or less using a written script, without developer intervention, critical defects, or manual database changes. | Dedicated Demo Vercel and clean Supabase boundaries are live from the same release. A namespaced, non-destructive one-click run command passes local role, environment, version, idempotency, isolation, and append-only database replay. Applying its migration and exact presenter allowlist to Demo, branded DNS, authenticated three-size browser verification, and an independent timed rehearsal remain |
| P0 | Independent persona UAT | Institution owner, account holder, representative, and reviewer use separate browser profiles or devices. Each receives the right message, completes only authorized actions, sees the same final receipt, and sees the same later lifecycle change. Wrong-role, reused-link, stale-page, rejected, withdrawn, expired, and recovery paths leave correct durable state and activity. | Fresh Demo owner, principal, and representative browser journey passed through private uploads, decision, matching receipts, and revocation. A separately authenticated reviewer plus the remaining negative matrix remain |
...
| Demo | Stable public entry, resettable approved test data, and a seven-minute script | Dedicated isolated app/database are live at the stable Vercel fallback; namespaced fresh-run provisioning passes locally, while branded DNS and deployed verification remain | Add the Cloudflare record, apply/configure the fresh-demo command in Demo, verify it at all three target widths, and rehearse without developer intervention |
