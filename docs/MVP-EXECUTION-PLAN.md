...
| P0 | Independent persona UAT | Institution owner, account holder, representative, and reviewer use separate browser profiles or devices. Each receives the right message, completes only authorized actions, sees the same final receipt, and sees the same later lifecycle change. Wrong-role, reused-link, stale-page, rejected, withdrawn, expired, and recovery paths leave correct durable state and activity. | Fresh Demo owner, principal, and representative browser journey passed through private uploads, correction recovery, decision, matching receipts, and revocation. An independent database read matched status, versions, events, decision fingerprint, and usage. Durable automated proof covers the full seven-case negative matrix. A separately authenticated reviewer, independent physical-profile rehearsal, recovery inbox placement, and hosted expiration initiation remain |
