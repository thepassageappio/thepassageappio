...
- Operational Calm hardening sprint, only where it supports task-spine demo readiness:
  ...
  - P2: favicon and console hygiene — VERIFIED RESOLVED 2026-07-25 (re-check before reopening): `pages/_app.js` already sets `<link rel="icon">`, `rel="shortcut icon"`, and `rel="apple-touch-icon"` from `PASSAGE_BRAND.assets` (`lib/brand.js`), and every referenced file (`public/favicon.svg`, `public/brand/passage-held-192.png`, `public/brand/passage-held-1024.png`) exists in `public/`. Separately, the "repeated Supabase client warnings" half of this item is also resolved: `lib/supabaseBrowser.js` exports a correct singleton (`getSupabaseBrowserClient()` caches one instance at module scope), and every browser-facing file in `pages/`, `pages/auth/`, `pages/system/`, `pages/vendors/`, and `components/` (including the legacy `App.js`, `LegacyParticipating.js`, `LegacyVendorRequest.js`) imports that one shared `supabase` export rather than calling `createClient()` itself. The only other `createClient()` call site, `lib/deliveryAuth.js`, runs server-side only inside API routes and cannot trigger the browser's "Multiple GoTrueClient instances" warning; per-invocation service-role clients in `pages/api/*` are the normal serverless pattern, not the bug this item described. No code change is needed. Leaving this line rather than deleting it so a future session doesn't silently lose the record and re-investigate the same non-issue from scratch.
  - P2: console hygiene beyond the above (unrelated warnings, if any) — not audited by this pass; re-open a fresh, narrower ticket if a specific warning is observed live.
