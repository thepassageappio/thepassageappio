...
Purpose: prove that Passage can support a real funeral-home pilot without founder narration. Run this before every funeral-home demo, pilot invite, or release that touches tasks, notifications, partner onboarding, exports, staff scope, vendor requests, or family updates.
...
5. Assign work to staff.
6. Move one task to waiting or handled with proof.
...
- Screenshot of task owner/waiting/proof state.
...
- Each task shows ask, owner, waiting point, proof destination, and notification state.
...
- Screenshot of task action form.
...
Pass criteria:
- Participant sees one scoped task and understands it in under 10 seconds.
...
- Task handled/waiting states can be saved without meaningful detail.
