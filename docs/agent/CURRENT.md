...
- The current exact hosted workspace replay passes at both 390px and 360px on production merge `1611c5dce4392a0674890896b7141a868935cd39`. The document stays within the viewport, role navigation remains in an explicit horizontal scroll lane, and every visible main-content interactive target measures at least 44px after PR #106 raised table controls from 40px.
...
1. **Finish presenter-grade release QA.** The post-MFA persona and exact mobile target-size replays pass; now complete a timed run, deterministic invitation-delivery fallback, keyboard/accessibility replay, and the hosted negative-path set.
