...
- The sample-workflow access mismatch is fixed on the current release branch: marketing sample links now use `/sample` instead of creating an Owner workspace and forcing MFA. Google remains the primary configured sign-in; email is the fallback. After authentication, a first-time viewer must explicitly choose `Agree and view sample`. That action writes one private append-only consent record, one commercial event, and one idempotent HubSpot Contact outbox job before access is granted. It creates no organization membership or capability, while institution Owner/Admin work remains behind AAL2. The local end-to-end and 390px browser replay passed, and the additive migration is applied and permission-verified in UAT and Demo. Application deployment and a real projection into Passage HubSpot portal `246159600` remain open because its deployed private-app credentials are not configured. See [../SAMPLE-WORKFLOW-ACCESS-DECISION-2026-09-08.md](../SAMPLE-WORKFLOW-ACCESS-DECISION-2026-09-08.md).
