...
This is a separate candidate after PR #144. Do not apply this migration while the old inline sender is active: both paths could send invitations. Quiesce new submissions for the migration/application handover and verify the worker before reopening. No recovery migration or worker has been enabled on hosted projects during implementation.
