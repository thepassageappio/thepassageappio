...
export function taskOutputFor(task, context = {}) {
  ...
  if (task?.playbook?.actionResultLabel) {
    return {
      ...
      body: task.playbook?.whatPassageDoes || task.playbook?.automationExplanation || 'Passage prepares the task output, keeps the next action visible, and records proof when someone moves it.',
    };
  }
  return {
    label: 'Task output and proof trail',
    body: `Passage prepares the next step for ${caseName}, tracks who owns it, and keeps proof visible to the right people.`,
  };
}
...
export function taskProofDestination(task, context = {}) {
  ...
  return `Saved updates appear in ${surface}, the communication/proof log, the family status view, reports, and export for "${title}".`;
}
