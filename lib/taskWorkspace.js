...
function operatingStatus(task) {
  const status = String(task?.status || task?.delivery_status || task?.outcome_status || '').toLowerCase();
  if (['handled', 'completed', 'done', 'not_applicable', 'cancelled'].includes(status) || task?.completed_at || task?.handled_at) return 'done';
  if (['blocked', 'failed', 'needs_review'].includes(status)) return 'needs_help';
  if (['waiting', 'pending', 'sent', 'delivered', 'assigned', 'acknowledged'].includes(status)) return 'waiting';
  return 'ready';
}

function automationContractFor(task) {
  const explicit = String(task?.automation_level || task?.automationLevel || task?.playbook?.automationLevel || '').toLowerCase();
  const label = explicit.includes('auto') && !explicit.includes('semi')
    ? 'Automated'
    : explicit.includes('semi')
      ? 'Semi-automated'
      : task?.playbook?.automationLabel || task?.playbook?.automationShortLabel
        ? 'Semi-automated'
        : 'Manual guidance';
  const reason = label === 'Automated'
    ? 'Passage can complete this safely from known data and logged system events.'
    : label === 'Semi-automated'
      ? 'Passage prepares the message, output, route, or proof, then a human reviews or confirms it.'
      : 'A person still needs to make the decision, contact someone, or add missing proof before Passage can do more.';
  const nextImprovement = label === 'Automated'
    ? 'Monitor quality, exceptions, and delivery proof.'
    : label === 'Semi-automated'
      ? 'Capture repeatable rules, required fields, and approval thresholds so more of this can run automatically later.'
      : 'Add structured fields, trusted recipients, templates, and approval rules to make this semi-automated.';
  return { label, reason, nextImprovement };
}


function communicationContractFor(task, context = {}, persona = 'family') {
  const title = taskDisplayTitle(task || {});
  const caseName = context.caseName || context.estateName || 'this family record';
  const waitingOn = task?.waiting_on || task?.playbook?.waitingOn || task?.recipient_name || task?.recipient || '';
  const assigned = task?.assigned_to_name || task?.assigned_to_email || task?.owner_name || '';
  const explicitAudience = task?.communication_audience || task?.audience || task?.recipient_name || task?.recipient || task?.playbook?.communicationAudience;
  const audience = explicitAudience || (persona === 'vendor'
    ? 'Vendor contact'
    : persona === 'care'
      ? 'Care handoff team'
      : persona === 'participant'
        ? 'Scoped helper or participant'
        : persona === 'director' || persona === 'staff'
  ...
  const reviewBoundary = persona === 'director' || persona === 'staff'
    ? 'Prepared for review. Save to the case first; send only when the funeral-home team approves the audience and wording.'
    : persona === 'vendor'
      ? 'Scoped to this vendor request. The vendor can respond without seeing the full family record.'
      : persona === 'care'
        ? 'Scoped to the care handoff. Share only the authority, release, timing, and contact details needed for the transition.'
        : persona === 'participant'
          ? 'Scoped to one request. The participant sees only what they need to help.'
          : 'Prepared for review. Nothing sends unless the family or authorized owner approves it.';
  const proofState = 'Logs prepared/copied/sent/replied states, audience, owner, timestamp, and proof on the case record.';
  return { audience, relatedContext, reviewBoundary, proofState };
}

export function taskOperatingContractFor(task, context = {}) {
  const title = taskDisplayTitle(task || {});
  const output = taskOutputFor(task, context);
  const requestDraft = taskRequestDraftFor(task, context);
  const proofDestination = taskProofDestination(task, context);
  const guidance = taskGuidanceFor(task, context);
  const status = operatingStatus(task);
  const owner = task?.assigned_to_name || task?.assigned_to_email || task?.owner_name || task?.playbook?.defaultOwner || context.owner || 'Assign an owner';
  const waitingOn = task?.waiting_on || task?.playbook?.waitingOn || task?.recipient_name || task?.recipient || (status === 'waiting' ? owner : 'No one yet');
  const role = String(context.role || 'family').toLowerCase();
  const persona = role.includes('director')
    ? 'director'
    : role.includes('staff') || role.includes('employee')
      ? 'staff'
      : role.includes('vendor')
        ...
        : role.includes('care')
          ...
          : role.includes('participant')
            ? 'participant'
            : 'family';
  const automation = automationContractFor(task);
  const communication = communicationContractFor(task, context, persona);
  const visibility = task?.visibility_label || task?.visibility || task?.playbook?.visibility || (persona === 'vendor' || persona === 'participant' ? 'Only this scoped request' : persona === 'care' ? 'Care handoff team' : 'Family and funeral home');
  const whatYouDo = status === 'done'
    ? 'Review the saved proof or export it if someone asks what happened.'
    : status === 'needs_help'
      ? 'Name the stuck point, ask the right person for the missing detail, and keep the case visible until it moves.'
      : status === 'waiting'
        ? `Follow up with ${waitingOn || 'the waiting person'}, or record what is still missing.`
        : guidance.nextStep || 'Confirm the owner, move the next action, and save proof.';
  const primaryAction = status === 'done'
    ? { label: 'View proof', status: 'view', tone: 'secondary' }
    : status === 'needs_help'
      ? { label: 'Request missing info', status: 'blocked', tone: 'warning' }
      : status === 'waiting'
        ? { label: 'Update waiting status', status: 'waiting', tone: 'secondary' }
        : { label: 'Save proof + close', status: 'handled', tone: 'primary' };
  const reassurance = persona === 'family'
    ? 'You only need the next clear ask. Passage keeps the rest organized.'
    : persona === 'vendor'
      ? 'You only see the scoped request, timing, quote state, and proof needed for your work.'
      : persona === 'care'
        ? 'This keeps the handoff clear without turning the care team into funeral-home operators.'
        : persona === 'participant'
          ? 'This is one scoped request. You do not need the full family record.'
          : 'This keeps the family relationship clear: one owner, one waiting point, one proof trail.';
  return {
    title,
    status,
    statusLabel: status === 'done' ? 'Done' : status === 'needs_help' ? 'Needs help' : status === 'waiting' ? 'Waiting' : 'Ready',
    owner,
    waitingOn,
    visibility,
    communication,
