...
export function taskExplanationFor(task, context = {}) {
  ...
  return {
    ...
    done: `${output.label || 'The prepared output'} is reviewed, the owner or waiting point is clear, and proof is saved before the task is closed.`,
  };
}
...
export function taskGuidanceFor(task, context = {}) {
  ...
  if (lower.includes('employer')) {
    return {
      ...
      overwhelmed: 'Send one HR request and mark the task waiting until the employer replies.',
    };
  }
  if (lower.includes('attorney') || lower.includes('probate') || lower.includes('executor')) {
    return {
