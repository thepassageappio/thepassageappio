...
export const TASK_ACTIONS = {
  accept: {
    ...
    confirmation: 'Saved. Everyone can see that you own this next step.',
    requiresNote: false,
  },
  handled: {
    status: 'handled',
    ...
    confirmation: 'Proof saved. This next step is marked done.',
    requiresNote: true,
  },
  waiting: {
    status: 'waiting',
  ...
  blocked: {
    ...
    confirmation: 'Help request saved. The needs-help status is visible now.',
    requiresNote: true,
  },
  not_applicable: {
...
export function taskActionConfirmation(action, item, role = 'family') {
  ...
  const title = textOf(item);
  if (role === 'participant') {
    if (normalized === 'accept') return 'Saved. The coordinator can see that you own this request.';
    if (normalized === 'handled') return 'Proof saved. This request is marked done for the coordinator.';
    if (normalized === 'waiting') return 'Waiting update saved. The coordinator can see what this request needs next.';
    if (normalized === 'blocked') return 'Help request saved. The coordinator can see what is needed.';
  }
  if (normalized === 'handled' && title.includes('pronouncement')) {
...
export function taskActionEventTitle(action, actorLabel = 'Participant') {
  const status = taskActionStatus(action);
  if (status === 'handled') return actorLabel + ' saved proof and completed a request';
  if (status === 'waiting') return actorLabel + ' marked a request waiting';
  if (status === 'acknowledged') return actorLabel + ' accepted a request';
  if (status === 'blocked') return actorLabel + ' needs help';
  return actorLabel + ' updated a request';
}
