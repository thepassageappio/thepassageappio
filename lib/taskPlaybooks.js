...
export const AUTOMATION_LEVELS = {
  SEND_TRACK: {
    ...
    shortLabel: 'Tier 1',
    explanation: 'Passage prepares the message, sends it after approval, and tracks confirmation.',
  },
  PARTNER_HANDOFF: {
    level: 2,
    tier: 'Assisted execution',
    label: 'Funeral-home or local-support handoff',
    shortLabel: 'Tier 2',
    explanation: 'Passage prepares the handoff and tracks proof, but a person, funeral home, or local support provider still completes the work.',
  },
  PREP_TRACK: {
    level: 2,
    tier: 'Assisted execution',
    label: 'Passage prepares and tracks this',
    shortLabel: 'Tier 2',
    explanation: 'Passage creates the packet or request, shows where it goes, and keeps proof/follow-up visible.',
  },
  PACKET: {
    level: 2,
    tier: 'Assisted execution',
...
export const EXECUTION_MODES = {
  automate: {
    ...
    failureRule: 'If delivery fails, Passage keeps the request open so it can be retried.',
  },
  call: {
  ...
  guide: {
    ...
    followUpRule: 'If the outside agency is still pending, Passage keeps the request waiting.',
  ...
  record: {
    ...
    followUpRule: 'Missing fields stay visible as future waiting items instead of blocking progress.',
...
const base = {
  ...
  officialLink: '',
  officialLinkLabel: '',
  deliverable: 'Prepared next step, owner, instructions, and proof field',
};
...
function executionModeFor(playbook) {
  ...
  if (playbook.executionKind === 'packet' || playbook.automationLevel === 'PACKET' || playbook.automationLevel === 'PREP_TRACK') return 'prepare';
  return 'automate';
}

function executionTruthFor(playbook, level, execution) {
  const mode = execution.modeKey;
  const deliverable = playbook.deliverable || '';
  if (mode === 'automate' && playbook.executionKind === 'message' && playbook.automationLevel === 'SEND_TRACK') {
    return {
      tier: 'Fully automated',
      shortLabel: 'Automate',
      label: 'Message prepared, sent, and tracked',
      explanation: 'Passage generates the message, sends it after approval, logs delivery, and keeps confirmation visible.',
      resultLabel: 'Action result: approved message plus delivery/proof trail',
  ...
  if (mode === 'prepare') {
    return {
      tier: 'Assisted execution',
      shortLabel: 'Packet',
      label: 'Packet or request prepared',
      explanation: 'Passage produces the packet/request and tracks proof, but a person still reviews, submits, or confirms it.',
      resultLabel: deliverable ? 'Action result: ' + deliverable : 'Action result: prepared packet/request and proof tracker',
      hasActionResult: true,
    };
  }
  if (mode === 'call') {
    return {
      tier: 'Assisted execution',
      shortLabel: 'Call',
      label: 'Call script and proof tracking',
      explanation: 'Passage gives the call script, context, and proof field; completion depends on the call outcome.',
      resultLabel: 'Action result: call script plus saved outcome/proof',
      hasActionResult: true,
    };
  }
  if (mode === 'record') {
    return {
      tier: 'Guided manual',
      shortLabel: 'Record',
      label: 'Structured estate record',
      explanation: 'Passage saves this detail so it can feed packets, messages, summaries, and later waiting items.',
      resultLabel: 'Action result: structured estate record and proof note',
      hasActionResult: true,
    };
  }
  return {
    ...
    resultLabel: 'Action result: official guidance and saved proof',
    hasActionResult: mode === 'guide' || Boolean(playbook.officialLink),
  };
}
...
const byTitle = {
  ...
  'share the news on social media': {
    ...
    deliverable: 'Social announcement draft with approval and copy/share tracking',
  },
  'draft the obituary': {
    automationLevel: 'PACKET',
    executionKind: 'packet',
    ...
    deliverable: 'Obituary draft workspace with copy, approval, and submission proof',
  },
  ...
  'draft and submit the obituary': {
    ...
    deliverable: 'Obituary submission packet with draft, recipient, and confirmation tracker',
  },
  'notify the deceased s employer': {
    automationLevel: 'SEND_TRACK',
    executionKind: 'message',
  ...
  'select readings music and pallbearers': {
    ...
    deliverable: 'Service-participant checklist with readings, music, pallbearers, and approval proof',
  },
  'gather photos and memories for the service': {
    ...
    deliverable: 'Photo and memory collection message with received-materials tracker',
  },
  ...
  'notify primary bank and all financial institutions': {
    ...
    deliverable: 'Bank notification packet with account list, authority note, and proof checklist',
  },
  'contact all life insurance companies to file claims': {
    automationLevel: 'PREP_TRACK',
    executionKind: 'packet',
    ...
    deliverable: 'Life insurance claim request packet and claim-number tracker',
  },
  ...
  'notify pension and retirement account administrators': {
    ...
    deliverable: 'Retirement plan notice packet and administrator follow-up tracker',
  },
  'address health insurance for surviving family members': {
    ...
    deliverable: 'Health coverage transition request and next-step tracker',
  },
  'notify medicare and medicaid if applicable': {
    ...
    deliverable: 'Benefits notification checklist with official route and proof field',
  ...
  'contact veterans affairs if veteran': {
    ...
    deliverable: 'VA burial benefit packet and appointment/reference tracker',
  ...
  'cancel voter registration': {
    ...
    deliverable: 'Local election-office notice packet and confirmation tracker',
  ...
  'set up mail forwarding or hold with usps': {
    ...
    deliverable: 'Mail forwarding details packet and USPS confirmation tracker',
  ...
  'notify all credit card companies': {
    ...
    requiredDocuments: ['death certificate', 'proof of authority'],
    deliverable: 'Credit card issuer notice packet and closure tracker',
  },
  'contact the dmv to cancel the driver s license': {
    automationLevel: 'PREP_TRACK',
    executionKind: 'packet',
    ...
    deliverable: 'DMV license cancellation packet and state instruction tracker',
  ...
  'notify professional licensing boards if applicable': {
    ...
    deliverable: 'License board notice packet and board response tracker',
  },
  'apply for social security survivor benefits': {
    ...
    deliverable: 'SSA survivor benefits prep packet and appointment tracker',
  ...
  'file for pension and annuity survivor benefits': {
    ...
    deliverable: 'Pension/annuity survivor claim packet and claim-start tracker',
  },
  ...
  'update beneficiary designations on all accounts': {
    ...
    deliverable: 'Beneficiary update checklist and account-provider proof tracker',
  },
  'transfer vehicle titles with the dmv': {
    ...
    deliverable: 'Vehicle title transfer packet and DMV receipt tracker',
    officialLink: 'https://www.usa.gov/motor-vehicle-services',
    officialLinkLabel: 'Find state DMV',
  },
  ...
  'cancel all subscriptions and recurring services': {
    ...
    deliverable: 'Subscription cancellation tracker with message templates',
  },
  'memorialize or close social media accounts': {
    ...
    deliverable: 'Social platform request packet and submission tracker',
  },
  ...
  'notify the three credit bureaus': {
    ...
    requiredDocuments: ['death certificate', 'proof of authority'],
    deliverable: 'Credit bureau deceased-alert packet and confirmation tracker',
  ...
  'cancel or surrender the passport': {
    ...
    deliverable: 'Passport surrender packet and submission tracker',
  ...
  'review and update home and auto insurance': {
    ...
    deliverable: 'Home/auto policy update packet and agent response tracker',
  },
...
function fallbackForTitle(title) {
  ...
  if (lower.includes('funeral') || lower.includes('cemetery') || lower.includes('crematorium') || lower.includes('obituary')) {
    return { ...base, automationLevel: 'PARTNER_HANDOFF', partnerOwnerRole: 'funeral_home_director', funeralHomeEligible: true, waitingOn: 'funeral home or local support provider', proofRequired: 'provider update' };
  }
  if (lower.includes('bank') || lower.includes('insurance') || lower.includes('benefit') || lower.includes('credit')) {
    return { ...base, automationLevel: 'PREP_TRACK', executionKind: 'packet', waitingOn: 'institution', proofRequired: 'institution instructions recorded', deliverable: 'Institution request packet and response tracker' };
  }
...
export function getTaskPlaybook(title) {
  ...
  const truth = executionTruthFor(merged, level, execution);
  return {
    ...merged,
    playbookKey: key,
    automation: level,
    ...
    executionTier: truth.tier,
    topProofTask: TOP_PROOF_TASKS.includes(key),
    institutionTemplate: template,
    automationLabel: truth.label,
    automationShortLabel: truth.shortLabel,
    automationExplanation: truth.explanation,
    actionResultLabel: truth.resultLabel,
    hasActionResult: truth.hasActionResult,
...
export function partnerTaskPriority(task) {
  ...
  if (playbook.automationLevel === 'PREP_TRACK') return 3;
  if (playbook.automationLevel === 'PACKET') return 4;
  return 5;
}
