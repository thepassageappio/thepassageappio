...
const SELECT_COLUMNS = 'id, situation_category, person_name, person_location, person_timing, coordinator_name, coordinator_phone, coordinator_email, callback_notes, wants_callback, status, version, claimed_organization_id, workflow_id, submitted_at, claimed_at, case_created_at';
...
export async function loadUrgentIntakeQueue(client: SupabaseClient, organizationId: string): Promise<{ unclaimed: UrgentIntakeRequest[]; mine: UrgentIntakeRequest[] } | null> {
  const unclaimedResult = await client.from('urgent_intake_requests').select(SELECT_COLUMNS).eq('status', 'submitted').order('submitted_at', { ascending: true });
  if (unclaimedResult.error) return null;
  const mineResult = await client.from('urgent_intake_requests').select(SELECT_COLUMNS).eq('claimed_organization_id', organizationId).order('submitted_at', { ascending: false });
