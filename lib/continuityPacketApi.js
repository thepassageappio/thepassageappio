...
const PACKET_ALIASES = {
  ...
  secure_home_assets: 'secure-home-assets',
  home_assets: 'secure-home-assets',
  vendor_request: 'vendor-service-request',
  vendor_quote_request: 'vendor-service-request',
  flowers_catering: 'vendor-service-request',
  obituary_service: 'obituary-service-packet',
  obituary_packet: 'obituary-service-packet',
};
...
export async function persistPreparedPacket({ estateId, taskId, packet, packetType, user, context }) {
  const admin = adminClient();
  if (!admin || !estateId || /^demo[-_]?/i.test(String(estateId))) {
    return { record: preparedPacketRecord({ estateId, taskId, packet, packetType, user, context }), persisted: false, reason: 'review_only' };
  }

  const record = preparedPacketRecord({ estateId, taskId, packet, packetType, user, context });
  const { data, error } = await admin
    ...
    .select('id,estate_id,task_id,type,version,generated_at,generated_by,data,file_path,status,created_at')
    .maybeSingle();
  ...
  if (error) {
    return {
      record: {
        ...
        persistence_note: 'Document packet table is not active in this environment; the reviewed output is still available to save as task proof.',
      },
      persisted: false,
  ...
  return {
    record: {
      ...record,
      ...data,
      persistence: 'document_packet_saved',
      fileName: packetFileName(packet),
      delivery: 'review_only',
    },
    persisted: true,
    reason: 'document_packets',
  };
}

export async function loadPersistedPacketRecords({ estateId, user, fallbackPackets = [], context }) {
  const admin = adminClient();
  if (!admin || !estateId || /^demo[-_]?/i.test(String(estateId))) return null;
  const { data, error } = await admin
    .from('document_packets')
    .select('id,estate_id,task_id,type,version,generated_at,generated_by,data,file_path,status,created_at')
  ...
  if (Array.isArray(data) && data.length) {
    return data.map(row => ({
      ...
      persistence: 'document_packet_saved',
      delivery: 'review_only',
      fileName: packetFileName({ title: row?.data?.title || row.type || row.id }),
    }));
  }

  return fallbackPackets.map(packet => preparedPacketRecord({
    estateId,
    packet,
...
function inputFromWorkflow({ workflow, organization, tasks = [], serviceEvents = [] }) {
  ...
  return {
    ...
    careTeamType: warm.provider_type || warm.care_provider_type || '',
    careSetting: warm.care_setting || '',
    careTeamName: warm.care_team_name || warm.facility_name || warm.hospice_agency || warm.hospiceAgency || '',
    careTeamContact: warm.care_team_contact || warm.facility_contact || warm.hospice_contact || warm.hospiceContact || warm.hospital_or_hospice_contact || '',
    careTeamPhone: warm.care_team_phone || warm.facility_phone || warm.hospice_phone || warm.hospicePhone || '',
    facilityName: warm.facility_name || '',
    facilityContact: warm.facility_contact || '',
    facilityPhone: warm.facility_phone || '',
...
export function preparedPacketRecord({ estateId, taskId, packet, packetType, user, context }) {
