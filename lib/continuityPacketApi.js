...
export async function persistPreparedPacket({ estateId, packet, packetType, user, context }) {
  const admin = adminClient();
  if (!admin || !estateId || /^demo[-_]?/i.test(String(estateId))) {
    return { record: preparedPacketRecord({ estateId, packet, packetType, user, context }), persisted: false, reason: 'review_only' };
  }

  const record = preparedPacketRecord({ estateId, packet, packetType, user, context });
  const { data, error } = await admin
    ...
    .select('id,estate_id,type,version,generated_at,generated_by,data,file_path,status,created_at')
    .maybeSingle();
