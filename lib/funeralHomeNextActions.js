...
function ctaForPriority(priority, stage) {
  if (priority === 'urgent') return 'Open next action now';
...
export function recommendedFuneralHomeNextAction({ caseItem, task = null, role = 'director', hasCases = true, now = Date.now() } = {}) {
  ...
  if (!caseItem) {
    return recommend({
      ...
      stage: 'queue',
      cta: 'Review queue',
      draft: 'Choose the next case, record what is done, waiting, or blocked, and leave it with a named owner before moving on.',
    }, { caseItem, task, role });
  }
  ...
  if (blocked) {
    return recommend({
      ...
      priority: 'urgent',
      cta: 'Resolve blocker',
      draft: text(blocked.request_draft) || `${taskTitle(blocked)} is blocked for ${name}. Name the decision owner, the smallest next step, and the next update time.`,
    }, { caseItem, task: blocked, role });
  }
