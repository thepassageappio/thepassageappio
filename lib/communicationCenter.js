...
export function taskExpectedUpdate(item, role = 'family') {
  ...
  if (isHandledStatus(status)) return `Handled - proof is saved on "${title}".`;
  if (status === 'blocked' || status === 'failed' || status === 'needs_review') {
    return role === 'funeral_home'
      ...
      : `Needs help - Passage is waiting for someone to unblock "${title}".`;
  }
  ...
  if (status === 'waiting' || status === 'pending' || status === 'assigned' || status === 'acknowledged') {
    if (role === 'funeral_home') {
      return waitingOn
        ? `Waiting on ${waitingOn} - check back by the next business morning or record the blocker.`
    ...
    return owner
      ...
      : `Waiting for an owner - Passage will show the next update here.`;
  }
...
export function buildCoordinationSpine({ tasks = [], statusEvents = [], communications = [], vendorRequests = [], estateEvents = [], limit = 12, role = 'family' } = {}) {
  ...
  (communications || []).forEach(message => {
    notifications.push({
      ...
      expectedUpdate: message.status === 'failed'
        ...
        : 'Delivery is tracked here; the task remains the source of truth.',
  ...
  (vendorRequests || []).forEach(request => {
    ...
    conversation.push({
      ...
      expectedUpdate: request.status === 'completed'
        ? 'Vendor work is completed; proof stays on the task.'
  ...
  return {
    ...buckets,
    attentionItems: unified.filter(rowIsActionable).slice(0, limit),
    latest: unified,
  };
}
