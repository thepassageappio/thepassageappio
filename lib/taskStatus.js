...
export async function recordStatusEvent({
  ...
  if (status === 'handled') taskUpdates.completed_at = now;
  ...
  if (status === 'handled') actionUpdates.handled_at = now;
  ...
  await serviceSupabase.from('estate_events').insert([{
    ...
    event_type: status === 'sent' ? 'task_message_sent' : status === 'failed' ? 'task_delivery_failed' : status === 'blocked' ? 'participant_blocked' : status === 'handled' ? 'task_completed' : 'task_status_updated',
    title: status === 'sent' ? 'Message sent' : status === 'failed' ? 'Message needs review' : status === 'blocked' ? 'Task needs help' : status === 'handled' ? 'Task handled' : 'Task status updated',
