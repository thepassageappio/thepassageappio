...
export async function recordStatusEvent({
  ...
  const terminalStatus = ['handled', 'completed', 'done'].includes(String(status || '').toLowerCase());
  const taskUpdates = {
    status,
    last_action_at: now,
  ...
  await serviceSupabase.from('estate_events').insert([{
    ...
    event_type: status === 'sent' ? 'task_message_sent' : status === 'failed' ? 'task_delivery_failed' : status === 'blocked' ? 'participant_blocked' : terminalStatus ? 'task_completed' : 'task_status_updated',
    title: status === 'sent' ? 'Message sent' : status === 'failed' ? 'Message needs review' : status === 'blocked' ? 'Task needs help' : terminalStatus ? 'Task handled' : 'Task status updated',
