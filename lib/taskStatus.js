...
export async function recordStatusEvent({
  ...
  if (storedStatus === 'delivered') actionUpdates.delivered_at = now;
  if (storedStatus === 'acknowledged') actionUpdates.acknowledged_at = now;
  ...
  await serviceSupabase.from('estate_events').insert([{
    ...
    event_type: storedStatus === 'sent' ? 'task_message_sent' : storedStatus === 'failed' ? 'task_delivery_failed' : storedStatus === 'blocked' ? 'participant_blocked' : terminalStatus ? 'task_completed' : 'task_status_updated',
    title: storedStatus === 'sent' ? 'Message sent' : storedStatus === 'failed' ? 'Message needs review' : storedStatus === 'blocked' ? 'Task needs help' : terminalStatus ? 'Task handled' : 'Task status updated',
    description: detail || ((recipient || 'Recipient') + ' - ' + storedStatus),
    actor: lastActor,
  }]).then(() => {}, () => {});
