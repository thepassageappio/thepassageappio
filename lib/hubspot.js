...
function routedDealDefaults({ pipeline, dealstage, persona, source, eventType }) {
  const route = String(eventType || source || persona || '').toLowerCase();
  const role = String(persona || '').toLowerCase();

  if (pipeline || dealstage) return { pipeline, dealstage };
  if (route.includes('checkout_completed') || route.includes('stripe')) {
    return {
      pipeline: process.env.HUBSPOT_D2C_PIPELINE || process.env.HUBSPOT_DEFAULT_PIPELINE,
      dealstage: process.env.HUBSPOT_D2C_CLOSED_WON_STAGE || process.env.HUBSPOT_CLOSED_WON_DEALSTAGE || process.env.HUBSPOT_DEFAULT_DEALSTAGE,
    };
  }
  if (route.includes('vendor') || role.includes('vendor')) {
    return {
      pipeline: process.env.HUBSPOT_VENDOR_PIPELINE || process.env.HUBSPOT_DEFAULT_PIPELINE,
      dealstage: process.env.HUBSPOT_VENDOR_APPLIED_STAGE || process.env.HUBSPOT_DEFAULT_DEALSTAGE,
    };
  }
  if (route.includes('care_provider') || route.includes('hospice') || route.includes('care') || role.includes('care_provider')) {
    return {
      pipeline: process.env.HUBSPOT_CARE_PIPELINE || process.env.HUBSPOT_DEFAULT_PIPELINE,
      dealstage: process.env.HUBSPOT_CARE_NEW_STAGE || process.env.HUBSPOT_DEFAULT_DEALSTAGE,
    };
  }
  if (route.includes('funeral_home') || role.includes('funeral_home')) {
    return {
      pipeline: process.env.HUBSPOT_FUNERAL_HOME_PIPELINE || process.env.HUBSPOT_DEFAULT_PIPELINE,
      dealstage: process.env.HUBSPOT_FUNERAL_HOME_NEW_STAGE || process.env.HUBSPOT_DEFAULT_DEALSTAGE,
    };
  }
  if (route.includes('web_lead') || role.includes('family') || role.includes('red_path') || role.includes('green_path')) {
    return {
      pipeline: process.env.HUBSPOT_D2C_PIPELINE || process.env.HUBSPOT_DEFAULT_PIPELINE,
      dealstage: process.env.HUBSPOT_D2C_NEW_STAGE || process.env.HUBSPOT_DEFAULT_DEALSTAGE,
    };
  }
  return {
    pipeline: process.env.HUBSPOT_DEFAULT_PIPELINE,
    dealstage: process.env.HUBSPOT_DEFAULT_DEALSTAGE,
  };
}

async function createDeal({ name, amount, pipeline, dealstage, persona, source, eventType, description, closeDate }) {
  const routed = routedDealDefaults({ pipeline, dealstage, persona, source, eventType });
  const properties = {
    dealname: clean(name, 220),
    amount: amount ? String(amount) : undefined,
    pipeline: routed.pipeline || 'default',
    dealstage: routed.dealstage || 'appointmentscheduled',
...
export async function syncLeadToHubSpot({ admin, eventType, source, sourceId, contact = {}, company = {}, deal = {}, payload = {} }) {
  ...
  try {
    ...
    const hsDeal = shouldCreateDeal ? await createDeal({ ...deal, source, eventType, persona: deal.persona || contact.persona || payload.persona }) : null;
