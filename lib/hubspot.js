...
export async function syncLeadToHubSpot({ admin, eventType, source, sourceId, contact = {}, company = {}, deal = {}, payload = {} }) {
  if (!enabled()) {
    await logCrmSync(admin, { source, eventType, sourceId, email: contact.email, companyName: company.name, status: 'skipped', error: 'Missing HUBSPOT_PRIVATE_APP_TOKEN', payload });
    return { skipped: true };
  }
  try {
