...
function taskTimingFit(task, context = {}) {
  ...
  const wait = reason => ({ ...timing, rankShift: 18, reason });
  ...
  if (phase === 'pre_death') {
    ...
    if (match(['death certificate', 'social security', 'bank', 'insurance claim', 'probate', 'dmv', 'passport', 'credit bureau', 'final income tax'])) {
