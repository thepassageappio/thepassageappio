...
export function taskStateMachine(task, allTasks = [], context = {}) {
  ...
  if (taskIsDone(task)) {
    ...
    escalation = 'Keep visible, but do not ask the family twice until the earlier step is complete.';
  } else if (!owner) {
    state = 'needs_owner';
    label = 'Needs owner';
...
export function orchestrateWorkflowStates(tasks = [], context = {}) {
  ...
  states.forEach(state => {
    ...
    state.reassurance = state.status === 'blocked_by_dependency'
      ? `This stage should wait until ${blockedDependencies.map(dep => dep.label).join(', ')} is ready.`
