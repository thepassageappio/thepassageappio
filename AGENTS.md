...
The durable policy is `docs/product/release-governance-and-plain-language-policy.md`. It supersedes any historical instruction that permits agents or schedules to ship directly to `main`, treats agent role separation as independent human review, allows overlapping greenfield initiatives to merge independently, or permits persona copy to expose internal implementation language.

- Agents and scheduled jobs must never push directly to `main`. All changes use a named branch and pull request. Production promotion requires passing checks and at least one approving review from a human GitHub account that neither authored nor materially implemented the change. PM/UX/Engineering/QA/Deploy separation remains mandatory, but it does not satisfy this human-review gate.
...
Agents and schedules never create that production release commit directly on `main`. The reviewed pull request is the unit of promotion. Repository rules should require checks, a non-author human approval, resolved conversations, stale-approval dismissal, restricted bypass/force-push, and serialized production releases.
