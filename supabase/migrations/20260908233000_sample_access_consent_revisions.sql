alter table authority_private.sample_access_leads
  drop constraint sample_access_leads_actor_user_id_key;

alter table authority_private.sample_access_leads
  add constraint sample_access_leads_actor_consent_version_key
  unique (actor_user_id, consent_version);
...
create or replace function authority_private.has_sample_access_lead_v2(
  p_actor_user_id uuid,
  p_consent_version text
)
returns boolean
...
create or replace function public.has_sample_access_lead_v2(
  p_actor_user_id uuid,
  p_consent_version text
)
returns boolean
...
stable
as $$
  select authority_private.has_sample_access_lead_v2(p_actor_user_id, p_consent_version);
$$;

revoke execute on function authority_private.has_sample_access_lead_v2(uuid, text) from public, anon, authenticated;
revoke execute on function public.has_sample_access_lead_v2(uuid, text) from public, anon, authenticated;
grant execute on function authority_private.has_sample_access_lead_v2(uuid, text) to service_role;
grant execute on function public.has_sample_access_lead_v2(uuid, text) to service_role;

comment on function public.has_sample_access_lead_v2(uuid, text) is
  'Service-only check that grants sample access only for the currently required consent version.';
