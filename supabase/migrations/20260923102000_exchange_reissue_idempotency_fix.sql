-- P0: receipt View after decision invite in same browser → session_unavailable.
-- Root cause: reissue_participant_invitation_v1 reuses invitation_id and revokes
-- sessions, but Path=/ invite-exchange idempotency cookie is reused. Exchange
-- finds the prior command receipt, cannot find an active session for the new
-- token derivation, and raises participant_session_unavailable.
-- Fix: when a command receipt exists but no active matching session and the
-- invitation is pending again (reissued), drop the stale receipt and mint a
-- fresh exchange. True double-submit replay (active session) unchanged.
...
begin
  ...
  -- Scope payload to this invitation generation so receipt/resume reissues are distinct.
  ...
  if found then
    ...
    -- Stale idempotency after reissue (same browser key, new token, sessions revoked).
    if v_invitation.status = 'pending' then
      delete from authority_private.participant_command_receipts
      where invitation_id = v_invitation.id
        and command_name = 'exchange_participant_invitation'
        and idempotency_key = p_idempotency_key;
    else
...
comment on function authority_private.exchange_participant_invitation_v1(text, uuid) is
  'Exchanges a one-time participant invitation for a role-bound session. Reuses an active session on true idempotent replay; after reissue (pending again, prior session gone) supersedes the stale command receipt so the same browser key can open the new token.';
