-- Passage Zero: Urgent/red family persona thin slice (Batch 3)
-- Adds the at-need first-response flow: a family member in crisis submits an
-- urgent intake (situation + person + callback contact), any owner/director
-- can claim it from an org-agnostic queue, then create a real case (workflow)
-- from it. Mirrors the vendor/partner thin slice's idempotent-RPC + append-only
-- event pattern exactly, with all three of that batch's discovered bugs
-- (RLS-helper grant, private-RPC grant, ambiguous `version` OUT variable)
-- avoided from the start.

do $$
begin
  if to_regclass('public.urgent_intake_requests') is not null then
    raise exception 'urgent_family_thin_slice already applied (public.urgent_intake_requests exists)';
  end if;
  if to_regclass('public.partner_requests') is null then
    raise exception 'Refusing to apply: expected partner_requests lineage not found. Wrong project?';
  end if;
...
create table public.urgent_intake_requests (
...
comment on table public.urgent_intake_requests is 'Passage Zero urgent/red family persona: at-need first-response intake. State machine: submitted|self_handling -> claimed -> case_created.';

create index urgent_intake_requests_requester_idx on public.urgent_intake_requests (requester_user_id);
create index urgent_intake_requests_status_idx on public.urgent_intake_requests (status);
create index urgent_intake_requests_claimed_org_idx on public.urgent_intake_requests (claimed_organization_id);

create table public.urgent_intake_events (
  id uuid primary key default gen_random_uuid(),
...
comment on table public.urgent_intake_events is 'Append-only audit trail for urgent_intake_requests lifecycle transitions. Mirrors partner_request_events.';

create index urgent_intake_events_request_idx on public.urgent_intake_events (urgent_intake_request_id, occurred_at);
...
-- ============================================================================
-- Authorization helpers (granted to authenticated immediately -- lesson from
-- the vendor/partner batch: a SECURITY DEFINER function still needs a direct
-- EXECUTE grant to be callable at all, regardless of what it does once entered).
-- ============================================================================
...
-- ============================================================================
-- RLS (read-only for clients; all writes go through SECURITY DEFINER RPCs)
-- ============================================================================
...
-- ============================================================================
-- RPC 1: submit_urgent_intake_idempotent (family submits; any signed-in user)
-- ============================================================================
...
-- ============================================================================
-- RPC 2: claim_urgent_intake_idempotent (any active owner/director claims)
-- ============================================================================
...
-- ============================================================================
-- RPC 3: create_case_from_urgent_intake_idempotent (director creates the case)
-- ============================================================================
