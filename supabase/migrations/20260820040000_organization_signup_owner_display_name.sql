-- Founder feedback 2026-08-20: funeral-home self-serve signup never asked
-- for the owner's name, so organization_members.display_name is null for
-- every self-serve org's owner -- it falls back to email/generic labels
-- everywhere a director's name is shown. Adds an optional display-name
-- param; email is deliberately NOT a separate field (it's already the
-- verified signed-in account's email -- asking for it again would just
-- invite a mismatch between who signed in and who the form claims owns it).
...
declare
  ...
  v_display_name text := nullif(btrim(coalesce(p_owner_display_name, '')), '');
  v_org_id uuid;
  v_location_id uuid;
  v_member_id uuid;
begin
  ...
  insert into public.organization_members (organization_id, user_id, email, display_name, role, status, accepted_at)
  values (v_org_id, v_actor_user_id, v_actor_email, v_display_name, 'owner', 'active', now())
...
set search_path to ''
as $$
  select * from passage_private.self_serve_create_organization(p_organization_name, p_location_name, p_city, p_state, p_owner_display_name)
$$;
