...
on public.estate_user_context for all to authenticated
using (
  user_id = auth.uid()
  or estate_id in (
    ...
    where w.user_id = auth.uid()
      ...
      or exists (select 1 from public.organization_members om where om.organization_id = w.organization_id and coalesce(om.status,'active')='active' and (om.user_id = auth.uid() or lower(om.email)=lower(auth.jwt() ->> 'email')))
  )
)
with check (
  user_id = auth.uid()
...
on public.obituaries for all to authenticated
using (
  user_id = auth.uid()
  or workflow_id in (
    select w.id from public.workflows w
    where w.user_id = auth.uid()
      or lower(w.coordinator_email) = lower(auth.jwt() ->> 'email')
      or exists (select 1 from public.estate_access ea where ea.workflow_id = w.id and coalesce(ea.status,'accepted') <> 'revoked' and (ea.user_id = auth.uid() or lower(ea.email)=lower(auth.jwt() ->> 'email')))
      or exists (select 1 from public.organization_members om where om.organization_id = w.organization_id and coalesce(om.status,'active')='active' and (om.user_id = auth.uid() or lower(om.email)=lower(auth.jwt() ->> 'email')))
  )
)
with check (
...
on public.orchestration_events for select to authenticated
using (
  workflow_id in (
    select w.id from public.workflows w
    where w.user_id = auth.uid()
...
on public.orchestration_events for insert to authenticated
with check (
  workflow_id in (
    select w.id from public.workflows w
    where w.user_id = auth.uid()
...
on public.urgent_sessions for all to authenticated
using (
  user_id = auth.uid()
  or estate_id in (
    select w.id from public.workflows w
...
with check (
  user_id = auth.uid()
  or estate_id in (select w.id from public.workflows w where w.user_id = auth.uid() or lower(w.coordinator_email) = lower(auth.jwt() ->> 'email'))
);
...
on public.workflow_announcements for all to authenticated
using (
  workflow_id in (
    select w.id from public.workflows w
    where w.user_id = auth.uid()
