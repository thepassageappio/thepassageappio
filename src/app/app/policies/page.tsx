...
import {
  ...
  offeredPublishedFinancialPoaPermissions,
...
export default async function PoliciesPage({ searchParams }: Props) {
  ...
  const { data: publishedRaw, error: publishedError } = await supabase.rpc("get_published_permission_catalog_v1", {
  ...
  const offered = offeredPublishedFinancialPoaPermissions(published);
  if (publishedError || !published?.published || !offered.length) {
    return <PolicyNotice title="We could not load your request rules" detail="Reload this page before starting or changing a request." retry />;
  }
  const versionLabel = published.published.version;
  const publishedAt = published.published.published_at ? formatEasternDateTime(published.published.published_at) : null;
          ...
          {canPublish && published?.published?.id ? (
            <div className={styles.panelActions}>
              ...
              <p style={{ margin: "8px 0 0", color: "var(--muted)", fontSize: 11, lineHeight: 1.5 }}>
                Open drafts keep their saved rules. Before sending an older draft, review the changes and save a new revision. Existing decisions do not change.
              </p>
            </div>
          ) : null}
        </section>
        <section className={styles.panel}>
          <div className={styles.panelHead}>
            <div>
              ...
              <p>
                You can save a new version of these two choices. Adding or changing choices is not available yet.
              </p>
            </div>
          </div>
