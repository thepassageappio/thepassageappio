// Local build only. Alternate browser network abort and invalid-key server preflight.
  ...
  for (const width of [1280, 390, 360, 320]) {
    ...
    await context.route('**/*', async route => {
      ...
      if (route.request().method() === 'POST') {
        ...
        proof.posts++;
        await new Promise(resolve => setTimeout(resolve, 400)); if (proof.posts % 2 === 1) return route.abort('internetdisconnected');
      }
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      assert.match(await form.getByRole('alert').innerText(), attempt === 1 ? /could not reach Passage/ : /Check the required fields/);
    ...
    if (width === 360) await page.screenshot({ path: 'work/contact-network-360.png', fullPage: true });
    await context.close();
  }
} finally { await browser.close(); }
assert.equal(proof.posts, 8);
mkdirSync('work', { recursive: true });
writeFileSync('work/contact-network-proof.json', JSON.stringify(proof, null, 2));
console.log('PASS: 4 aborted requests and 4 retry/preflight responses; values/key retained, pending and focus recovered. No database or provider sends.');
