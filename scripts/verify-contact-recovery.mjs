// Run only against a local production build with Supabase URL/service key unset.
...
try {
  for (const width of [1280, 390, 360, 320]) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      assert.match(await form.getByRole('alert').innerText(), /could not confirm.*saved/s);
    ...
    if (width === 360) await page.screenshot({ path: 'work/contact-recovery-360.png', fullPage: true });
    await context.close();
  }
} finally { await browser.close(); }
assert.equal(proof.posts, 8);
mkdirSync('work', { recursive: true });
writeFileSync('work/contact-recovery-proof.json', JSON.stringify(proof, null, 2));
console.log('PASS: 8 actual local Server Action failures/retries at four widths; values/key retained, pending lock, error focus, no overflow/page errors. No database configured or provider sends.');
