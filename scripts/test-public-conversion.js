...
const operationsFrame = read('components/operations/AppFrame.tsx');
const demoReset = read('app/demo/DemoReset.tsx');
const demoResetStyles = read('app/demo/DemoReset.module.css');
const operatorAvailability = read('lib/presentation/operator-demo-availability.ts');
...
const demoPassRoute = read('app/demo/family/pass/page.tsx');
const authenticatedPassRoute = read('app/family/pass/page.tsx');
...
const operationsShellStyles = read('components/operations/OperationsShell.module.css');
assert(fs.existsSync('lib/presentation/demo-handoff-snapshot.js'), 'versioned demo handoff snapshot normalizer is required');
const demoSnapshot = read('lib/presentation/demo-handoff-snapshot.js');
...
assert(demo.includes('<DemoReset operatorSessionsConfigured={operatorSessionsConfigured} />'));
assert(demo.includes('<TopShell context="Example workspace" mode="gateway" operatorSessionsConfigured={operatorSessionsConfigured}>'));
...
for (const href of ['/director', '/director/intake', '/staff', '/partner', '/receive']) assert(!operatorDemo.includes(`href="${href}"`), `guided operator packet links to protected route: ${href}`);
for (const href of ['/demo/operator/director', '/demo/operator/staff', '/demo/operator/vendor']) assert(operationsFrame.includes(`href="${href}"`));
assert(operationsFrame.includes("guidedDemoPersona ? '/demo'"));
const guidedNavStart = operationsFrame.indexOf('{guidedDemoPersona ? <>');
const guidedNavEnd = operationsFrame.indexOf('</> : <>', guidedNavStart);
const guidedNav = operationsFrame.slice(guidedNavStart, guidedNavEnd);
assert(guidedNavStart >= 0 && guidedNavEnd > guidedNavStart);
for (const href of guidedNav.matchAll(/href="([^"]+)"/g)) assert(href[1] === '/demo' || href[1].startsWith('/demo/'), `guided shell escapes demo containment: ${href[1]}`);
for (const href of operatorDemo.matchAll(/href="([^"]+)"/g)) assert(href[1] === '/demo' || href[1].startsWith('/demo/'), `guided operator footer escapes demo containment: ${href[1]}`);
const operatorNarrationWithoutPrivacyFact = operatorDemo.replaceAll('Not shared', '');
for (const misleading of ['director can see', 'Northstar can now', 'Northstar can see', 'team activity shows', 'saved for director review', 'server', 'real workspace', 'authorized', 'durable', 'shared']) assert(!operatorNarrationWithoutPrivacyFact.toLowerCase().includes(misleading.toLowerCase()), `guided operator packet implies internal or shared state: ${misleading}`);
for (const truth of ['This page now shows', 'changes stay on this page', 'resets when you refresh', 'Nothing is saved to a real record or sent to anyone.', 'No purchase or payment was created.']) assert(operatorDemo.includes(truth));
...
const skipRule = operationsShellStyles.match(/\.skip\s*\{([\s\S]*?)\}/)?.[1] ?? '';
for (const contract of ['display: inline-flex;', 'align-items: center;', 'justify-content: center;', 'min-width: 48px;', 'min-height: 48px;']) assert(skipRule.includes(contract), `focused operator skip link must retain ${contract}`);
assert(topShell.includes('OPERATOR CHANGES STAY ON THE CURRENT PAGE AND RESET ON REFRESH'));
assert(topShell.includes('TEAM EXAMPLES USE ISOLATED SERVER SESSIONS'));
assert(!topShell.includes('TEAM EXAMPLE ACTIVITY IS SHARED'));
assert(operatorAvailability.includes("process.env.PASSAGE_PREVIEW_DEMO_SESSIONS_ENABLED === 'true'"));
assert(operatorAvailability.includes("configuration.projectRef === 'uyacxqtsiwlvtmhxvoxr'"));
for (const persona of ['director', 'staff', 'vendor']) assert(operatorAvailability.includes(persona));
assert(demoAction.includes('hasConfiguredOperatorDemoSession(persona)'));

assert(demoReset.includes('Reset the family demo'));
assert(demoReset.includes('Guided operator changes exist only on their current page and reset on refresh.'));
assert(demoReset.includes('Isolated operator workspaces were not changed.'));
assert(!demoReset.toLowerCase().includes('shared director'));
...
assert(transferComposer.includes('createDemoHandoffSnapshot({'), 'activation must create one reviewed snapshot');
assert(transferComposer.includes('JSON.stringify(snapshot)'), 'activation must serialize the exact snapshot once');
assert(familyIntent.includes('intent={intent}'), 'the current selected intent must cross the journey boundary');
assert(transferComposer.includes('intent: props.intent'), 'activation must snapshot the current selected intent');
assert(transferComposer.includes("props.providerMode === 'browser_demo' ? createDemoHandoffSnapshot"), 'only the explicit browser-demo adapter may create the snapshot');
assert(transferComposer.includes(': JSON.stringify(activated);'), 'authenticated activation must preserve its legacy draft serialization');
assert(demoPassRoute.includes('<ActivePass mode="browser_demo" />'), 'demo pass must explicitly select the browser-demo projection');
assert(authenticatedPassRoute.includes('<ActivePass mode="authenticated" />'), 'authenticated pass must explicitly retain its existing projection');
...
assert(activePass.includes('normalizeDemoHandoffSnapshot'));
assert(activePass.includes('This example handoff is unavailable'));
assert(activePass.includes('Create it again'));
assert(!activePass.includes('FALLBACK_PASS'), 'browser demo must not retain the stale fallback pass');
assert(activePass.includes("if (mode === 'authenticated') {\n        if (stored)"), 'legacy normalization must remain behind the authenticated adapter');
assert(activePass.includes("const demoPass = mode === 'browser_demo'"), 'browser-demo projection must use only its normalized snapshot');
assert(activePass.includes("const heroStatus = mode === 'authenticated'\n    ? familyStatus.update"), 'authenticated pass must preserve its family status source');
assert(activePass.includes('`Your handoff is ready for ${recipient.organization}.`'), 'issued browser-demo hero must name the validated snapshot receiver');
assert(activePass.includes('`This browser example shows ${recipient.organization} received the handoff.`'), 'accepted browser-demo hero must name the validated snapshot receiver truthfully');
assert(activePass.includes('<span>{heroStatus}</span>'), 'hero must render the mode-bounded status instead of familyStatus directly');
assert(!activePass.includes('<span>{familyStatus.update}</span>'), 'browser-demo hero must not render the sandbox default receiver sentence');
for (const contract of ['schema: DEMO_HANDOFF_SCHEMA', 'version: DEMO_HANDOFF_VERSION', 'receiver:', 'scopeIds:', 'expiryId:', 'activatedAt:', 'expiresAt:']) assert(demoSnapshot.includes(contract), `snapshot normalizer missing ${contract}`);
