...
for (const misleading of ['director can see', 'Northstar can now', 'Northstar can see', 'team activity shows', 'saved for director review']) assert(!operatorDemo.toLowerCase().includes(misleading.toLowerCase()), `guided operator packet implies shared state: ${misleading}`);
for (const truth of ['This page now shows', 'after it is saved to the server', 'This example change exists only on this page and resets when you refresh.']) assert(operatorDemo.includes(truth));
